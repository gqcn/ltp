...
type LoginReq struct {
	...
	Mode     string `json:"mode" v:"required|in:admin,ldap" dc:"登录方式。admin 为平台管理员入口，ldap 为企业目录入口。" eg:"admin"`
	Username string `json:"username" v:"required" dc:"登录用户名或域账号" eg:"admin"`
	Password string `json:"password" v:"required" dc:"登录密码。管理员为本地密码，LDAP 用户为目录密码。" eg:"admin123"`
}
