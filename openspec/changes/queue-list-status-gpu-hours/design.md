...
6. **删除确认框文案对齐`prototype.v4`**
   - 空闲删除沿用原型问句「确定要删除队列 {显示名} 吗？」；后果条为「团队关联将解除。已结束任务的历史记录会保留。此操作不可撤销。」不写「新任务不可再选择该队列」（与禁用框重复，删除后队列不存在）。
   - 占用拦截：正文写入运行中/排队中数量（「仍有 N 个运行中、M 个排队中的任务，暂不可删除」）；提示条只保留处理建议「请等待任务结束，或先停止相关任务后再删除」，数量不在提示条重复。
