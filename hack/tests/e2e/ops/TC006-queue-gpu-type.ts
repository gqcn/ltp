...
test("TC006 queue gpu type dropdown omits cpu placeholder", async ({ page }) => {
  ...
  await expect.poll(async () => gpu.locator("option").count()).toBeGreaterThan(0);
  const values = await gpu.locator("option").evaluateAll((opts) => opts.map((el) => (el as HTMLOptionElement).value));
  const labels = await gpu.locator("option").evaluateAll((opts) => opts.map((el) => (el.textContent || "").trim()));
  expect(values).not.toContain("cpu");
  expect(labels.some((text) => text === "cpu" || text.startsWith("cpu（"))).toBeFalsy();
