...
(function () {
  ...
  /** 提交预览中超高板块的展开状态（按 data-fold） */
  let createSummaryFolds = Object.create(null);
  ...
  const CONFIG_TEMPLATES = {
    megatron: [
      ...
      { path: "data.yaml", kind: "config", content: "data_path: /data/jiangsu/mix-v3\nsplit: 98,2,0\n" },
  ...
  let queueMgmtFilter = { dc: "all", gpuType: "all", q: "", page: 1, pageSize: 10 };
  let userMgmtFilter = { status: "all", roleId: "all", q: "", page: 1, pageSize: 10 };
  /** 用户管理多选（username 集合） */
  ...
  let dcMgmtFilter = { status: "all", q: "", page: 1, pageSize: 10 };
  let editingDcId = null;
  let pendingDcAction = null; // { action: enable|disable|delete, dcId }
  ...
  /** 平台内置默认数据中心模板（始终保证存在） */
  function defaultDatacenterTemplate() {
    return {
      id: "default",
      name: "默认数据中心",
      short: "默认",
  ...
  /**
   * 确保始终存在默认数据中心，避免未配置时任务 / 节点 / 队列无法关联。
   * 返回当前默认数据中心对象。
   */
  function ensureDefaultDatacenter() {
    if (!Array.isArray(MOCK.datacenters)) MOCK.datacenters = [];
    let d = MOCK.datacenters.find((x) => x.isDefault) || MOCK.datacenters.find((x) => x.id === "default");
    if (!d) {
      d = defaultDatacenterTemplate();
      MOCK.datacenters.unshift(d);
    } else {
      d.isDefault = true;
      d.enabled = true;
      if (!d.id) d.id = "default";
      if (!d.name) d.name = "默认数据中心";
      if (!d.short) d.short = "默认";
      if (!d.labelKey) d.labelKey = "maip.io/datacenter";
      if (!d.label) d.label = buildDcLabel(d.labelKey, d.id);
      if (!d.color) d.color = "#64748b";
    }
    // 保证唯一默认
    MOCK.datacenters.forEach((x) => {
      if (x !== d) x.isDefault = false;
    });
    return d;
  }
  ...
  function defaultDc() {
    return ensureDefaultDatacenter();
  }

  function defaultDcId() {
    return defaultDc().id || "default";
  }
  ...
  function dcBadge(dcId) {
    ...
    const muted = d.enabled === false ? " is-muted" : "";
    const def = d.isDefault ? ' title="默认数据中心"' : "";
    return `<span class="dc-badge${muted}" style="--dc-color:${d.color || "var(--primary)"}"${def}>${escapeHtml(
      d.short || d.name
    )}</span>`;
  }
  ...
  function dcUsageStats(dcId) {
    const nodes = (MOCK.nodes || []).filter((n) => n.dc === dcId).length;
  ...
  function clusterSelectLabel(id) {
    ...
    if (!c) return id || "—";
    return c.displayName ? `${c.displayName}（${c.name}）` : c.name || c.id;
  }
  ...
  function openClusterSwitchConfirm(targetId) {
    ...
    if (metaEl) {
      const fromC = findCluster(fromId);
      const toC = findCluster(targetId);
      metaEl.textContent = `${fromC?.name || fromId || "—"}  →  ${toC?.name || targetId}`;
    }
  ...
  function goCreateJob() {
    rerunFromJobId = null;
    createFormTab = "basic";
    createSummaryFolds = Object.create(null);
    navigate("job-create");
  }
  ...
  function tbLogdirPathForJob(job) {
    const ownerSlug = (job.owner || "user").replace(/\s+/g, "").toLowerCase() || "user";
    // 演示路径：与方案约定 outputs/{job_id}/tensorboard
    return `/data/home/${ownerSlug}/outputs/${job.id}/tensorboard`;
  }
        ...
        ${items
          ...
          <tr class="my-queue-row ${enabled ? "" : "is-disabled-row"}${sortedJobs.length ? " has-jobs" : ""}">
            ...
            <td class="my-queue-cpu-mem-cell mono" title="CPU ${q.cpuUsed || 0}/${q.cpuQuota || 0} · 余 ${cpuFree} 核 · 内存 ${q.memUsedGi || 0}/${q.memQuotaGi || 0} Gi · 余 ${memFree} Gi">
              ${q.cpuUsed || 0}/${q.cpuQuota || 0} · ${formatMemPair(q.memUsedGi, q.memQuotaGi)}
            </td>
  ...
  function configSetHasSecret(set, ver) {
    const files = ver?.files || latestConfigVersion(set)?.files || [];
    return files.some((f) => f.kind === "secret");
  }
  ...
  function cloneConfigFiles(files) {
    return (files || []).map((f) => ({ path: f.path, kind: f.kind || "config", content: f.content || "" }));
  }

  function scanSecretHints(content) {
    const text = String(content || "");
    return /hf_[A-Za-z0-9]{16,}|AKIA[0-9A-Z]{16}|BEGIN (RSA |OPENSSH )?PRIVATE KEY/.test(text);
  }
  ...
  function defaultConfigMountPath(taskName) {
    return `/data/hpc/home/${currentConfigUsername()}/experiments/${jobNameForConfigPath(taskName)}/configs`;
  }
  ...
  function filterConfigSets() {
    ...
    return visibleConfigSets().filter((set) => {
      ...
      const hasSecret = configSetHasSecret(set, latest);
      if (configListState.kind === "secret" && !hasSecret) return false;
      if (configListState.kind === "config" && hasSecret) return false;
      ...
      if (q) {
        const fileHit = (latest?.files || []).some((f) => String(f.path || "").toLowerCase().includes(q));
        const blob = `${set.displayName} ${set.name} ${set.id} ${set.owner} ${set.description || ""}`.toLowerCase();
        if (!blob.includes(q) && !fileHit) return false;
      }
      return true;
    });
  }
  ...
  function renderConfigList() {
    ...
    $("#cfg-filter-kind") && ($("#cfg-filter-kind").value = configListState.kind);
          ...
          return `<tr class="${archived ? "cfg-row-archived" : ""}">
            <td>
              <div class="cfg-name-cell">
                ...
                <span class="cfg-id mono">${escapeHtml(set.name)}</span>
              </div>
            </td>
            <td>${
              hasSecret
                ? `<span class="badge cfg-kind-secret">含密钥</span>`
    ...
    bindFilter("#cfg-filter-kind", "kind");
  ...
  function initConfigEditor(set) {
    ...
    if (set) {
      const latest = latestConfigVersion(set);
      const sourceFiles = set.draft && set.draft.owner === MOCK.user?.name ? set.draft.files : latest?.files;
      configEditor = {
        mode: "edit",
        setId: set.id,
  ...
  function persistConfigEditorForm() {
    ...
    if (!ed) return;
    if ($("#cfg-edit-name")) ed.meta.name = $("#cfg-edit-name").value.trim() || "";
    ...
    if (cur) {
      ...
      if ($("#cfg-file-secret")) cur.kind = $("#cfg-file-secret").checked ? "secret" : "config";
    }
  }
  ...
  function syncConfigEditTabMeta() {
    const ed = configEditor;
    if (!ed) return;
    const name = ed.meta.displayName || ed.meta.name;
    ...
    setMeta("basic", name || "标识 / 团队 / 可见性", !!(ed.meta.name && ed.meta.displayName));
    setMeta("files", files ? `${files} 个文件` : "尚未添加文件", files > 0);
    setMeta("publish", msg || "发布前填写说明", !!msg);
  }

  function switchConfigEditTab(tab) {
    if (!configEditor) return;
    persistConfigEditorForm();
    if (!CFG_EDIT_TABS.includes(tab)) tab = "basic";
    configEditor.tab = tab;
    $$("#cfg-edit-tabs [data-cfg-edit-tab]").forEach((el) => {
      const on = el.dataset.cfgEditTab === tab;
      el.classList.toggle("active", on);
      el.setAttribute("aria-selected", on ? "true" : "false");
    });
    $$("#cfg-edit-panels > .tab-panel").forEach((p) => {
      p.classList.toggle("active", p.dataset.panel === tab);
    });
    const i = CFG_EDIT_TABS.indexOf(tab);
    const prev = $("#btn-cfg-edit-prev");
    const next = $("#btn-cfg-edit-next");
    if (prev) prev.disabled = i <= 0;
    if (next) {
      const last = i >= CFG_EDIT_TABS.length - 1;
      next.disabled = last;
      next.classList.toggle("hidden", last);
    }
    if (tab === "files") bindYamlEditor($("#cfg-file-content"), $("#cfg-file-hl"));
  ...
  function renderConfigEdit() {
    ...
    $("#cfg-edit-title").textContent = isNew ? "新建配置集" : `发布新版本 · ${ed.meta.name}`;
    $("#cfg-edit-desc").textContent = isNew
      ? "填写基本信息与文件，保存草稿或直接发布为 v1"
      : `基于 v${ed.baseVersion} 编辑，发布后成为不可变 v${nextVer}`;

    if (!ed.tab || !CFG_EDIT_TABS.includes(ed.tab)) ed.tab = isNew ? "basic" : "files";
    ...
    $("#cfg-edit-body").innerHTML = `
      <div class="cfg-editor-layout">
        <div class="card job-create-form-card cfg-edit-form-card">
          <div class="tabs create-form-tabs" id="cfg-edit-tabs" role="tablist" aria-label="配置表单">
            <button type="button" class="tab" role="tab" data-cfg-edit-tab="basic" aria-selected="false">
              <span class="num">1</span>
              <span class="create-tab-copy">
                <span class="create-tab-title">基本信息</span>
                <span class="create-tab-meta" data-cfg-edit-tab-meta="basic">标识 / 团队 / 可见性</span>
              </span>
            </button>
            <button type="button" class="tab" role="tab" data-cfg-edit-tab="files" aria-selected="false">
              <span class="num">2</span>
              <span class="create-tab-copy">
                <span class="create-tab-title">文件</span>
                <span class="create-tab-meta" data-cfg-edit-tab-meta="files">YAML / 密钥</span>
              </span>
            </button>
            <button type="button" class="tab" role="tab" data-cfg-edit-tab="publish" aria-selected="false">
          ...
          <div id="cfg-edit-panels">
            <div class="tab-panel" data-panel="basic" role="tabpanel">
              <div class="form-grid">
                <div class="form-group">
                  <div class="field-label-row">
                    <label>标识 name ${isNew ? '<span class="req">*</span>' : ""}</label>
                    ${isNew ? "" : '<span class="field-lock-hint">创建后不可改</span>'}
                  </div>
                  <input id="cfg-edit-name" class="mono" ${isNew ? "" : "readonly"} placeholder="例如 slm-7b-phase4" value="${escapeHtml(ed.meta.name)}" />
                </div>
                <div class="form-group">
            ...
            <div class="tab-panel" data-panel="files" role="tabpanel">
                        ...
                        ? ed.files
                            .map(
                              (f) => `<button type="button" class="cfg-file-item ${f.path === active?.path ? "is-active" : ""}${configFileOverLimit(f) ? " is-over" : ""}" data-cfg-file="${escapeHtml(f.path)}">
                                ...
                                ${f.kind === "secret" ? `<span class="cfg-file-secret">密钥</span>` : ""}
                  ...
                  ${
                    active
                      ? `<div class="cfg-file-editor-head">
                          ...
                          <label class="cfg-secret-toggle">
                            <input type="checkbox" id="cfg-file-secret" ${active.kind === "secret" ? "checked" : ""} />
                            <span>密钥文件</span>
                          </label>
            ...
            <div class="tab-panel" data-panel="publish" role="tabpanel">
              <div class="form-grid">
                <div class="form-group full">
          ...
          <div class="create-form-footer cfg-edit-footer">
            <div class="create-form-footer-nav">
              <button type="button" class="btn btn-secondary" id="btn-cfg-edit-prev">上一步</button>
              <button type="button" class="btn btn-secondary" id="btn-cfg-edit-next">下一步</button>
            </div>
    ...
    ["cfg-edit-name", "cfg-edit-display", "cfg-edit-team", "cfg-edit-vis", "cfg-edit-desc-input", "cfg-edit-message"].forEach(
      (id) => {
        const onChange = () => {
          persistConfigEditorForm();
          syncConfigEditTabMeta();
    ...
    $$("#cfg-edit-tabs [data-cfg-edit-tab]").forEach((el) => {
      el.addEventListener("click", () => switchConfigEditTab(el.dataset.cfgEditTab));
    });
    $("#btn-cfg-edit-prev")?.addEventListener("click", () => {
      const i = CFG_EDIT_TABS.indexOf(ed.tab || "basic");
      switchConfigEditTab(CFG_EDIT_TABS[Math.max(0, i - 1)]);
    });
    $("#btn-cfg-edit-next")?.addEventListener("click", () => {
      const i = CFG_EDIT_TABS.indexOf(ed.tab || "basic");
      switchConfigEditTab(CFG_EDIT_TABS[Math.min(CFG_EDIT_TABS.length - 1, i + 1)]);
    });
    ...
    $("#cfg-file-path")?.addEventListener("change", () => {
      if (!active) return;
      const next = $("#cfg-file-path").value.trim().replace(/^\/+/, "");
      if (!next) return;
      if (ed.files.some((f) => f !== active && f.path === next)) {
        toast("相对路径不能重复", "error");
        return;
      }
      active.path = next;
      ed.activePath = next;
    });
    $("#cfg-file-secret")?.addEventListener("change", (e) => {
      if (active) active.kind = e.target.checked ? "secret" : "config";
      persistConfigEditorForm();
      syncConfigEditTabMeta();
    });
    ...
    $("#btn-cfg-del-file")?.addEventListener("click", () => {
      if (!active) return;
      persistConfigEditorForm();
      ed.files = ed.files.filter((f) => f !== active);
      ed.activePath = ed.files[0]?.path || "";
      ed.tab = "files";
      renderConfigEdit();
    });
  ...
  function saveConfigDraft() {
    ...
    if (ed.mode === "create") {
      if (!ed.meta.name || !ed.meta.displayName) {
        focusConfigEditIssue("basic", "保存草稿前请填写 name 与显示名称");
        return;
      }
      if (!/^[a-zA-Z][a-zA-Z0-9._-]*$/.test(ed.meta.name)) {
        focusConfigEditIssue("basic", "name 需以字母开头，仅含字母数字 . _ -");
        return;
      }
      if (MOCK.configSets.some((s) => s.name === ed.meta.name && s.teamId === ed.meta.teamId)) {
        focusConfigEditIssue("basic", "同团队下 name 已存在");
        return;
      }
      const team = findTeam(ed.meta.teamId);
      const id = `cfg-${ed.meta.name}`;
      const set = {
        id,
        name: ed.meta.name,
  ...
  function publishConfigVersion() {
    ...
    if (!assertConfigFilesWithinLimit()) return;
    if (!ed.meta.name || !ed.meta.displayName) {
      focusConfigEditIssue("basic", "请填写 name 与显示名称");
      return;
    }
    if (!ed.files.length) {
    ...
    for (const f of ed.files) {
      if (scanSecretHints(f.content) && f.kind !== "secret") {
        focusConfigEditIssue("files", `${f.path} 疑似包含密钥，已阻止以普通配置发布。请标为密钥文件或删掉明文 token`);
        return;
      }
    }
    if (ed.mode === "create") {
  ...
  function renderConfigDetail() {
    ...
    const latest = latestConfigVersion(set);
    const viewVer = findConfigVersion(set, set._viewVersion || latest?.version);
    const hasSecret = configSetHasSecret(set, viewVer);
    $("#cfg-detail-hero").innerHTML = `
      <div class="detail-hero-top">
        <div>
          ...
          } ${hasSecret ? '<span class="badge cfg-kind-secret">含密钥</span>' : ""}</h2>
          <div class="mono text-muted mt-8" style="font-size:12px">${escapeHtml(set.name)} · ${escapeHtml(set.id)}</div>
        </div>
    ...
    initTabs("#cfg-detail-tabs", "#cfg-detail-panels", (t) => {
      if (t === "files") renderConfigDetailFiles(set, viewVer);
  ...
  function renderConfigDetailFiles(set, ver) {
    ...
    $("#panel-cfg-files").innerHTML = `
      <div class="cfg-preview-meta">
        <div class="text-muted" style="font-size:12.5px">
          正在查看 <strong>v${ver?.version ?? "—"}</strong>
          ${ver?.message ? ` · ${escapeHtml(ver.message)}` : ""}
        </div>
        <div class="mono text-muted" style="font-size:12px">${files.length} 文件 · ${formatBytes(configVersionBytes(ver))}</div>
      </div>
      <div class="cfg-tree-split">
        <div class="cfg-file-tree" style="min-height:240px;border:1px solid var(--border);border-radius:10px">
            ...
            ${files
              .map(
                (f) => `<button type="button" class="cfg-file-item ${f.path === active?.path ? "is-active" : ""}" data-cfg-view-file="${escapeHtml(f.path)}">
                  <span class="cfg-file-path">${escapeHtml(f.path)}</span>
                  ${f.kind === "secret" ? `<span class="cfg-file-secret">密钥</span>` : ""}
                </button>`
              )
              .join("")}
        ...
        <div>
          ${
            active
              ? `<div class="cfg-preview-meta">
                   <span class="mono">${escapeHtml(active.path)}</span>
                   <span class="text-muted" style="font-size:12px">${formatBytes(configFileBytes(active))}</span>
                 </div>
                 ${
                   active.kind === "secret" && !set._reveal
                     ? codeBlockHtml("********\n# 密钥内容默认脱敏。点「显示明文」二次确认后可见。", "yaml")
                     : codeBlockHtml(active.content || "", langFromPath(active.path))
                 }
                 ${
                   active.kind === "secret"
                     ? `<button type="button" class="btn btn-secondary btn-sm mt-8" id="btn-cfg-reveal">${set._reveal ? "隐藏明文" : "显示明文"}</button>`
                     : ""
                 }`
              : `<div class="empty-state">无文件</div>`
          }
        </div>
      </div>`;
    ...
    $("#btn-cfg-reveal")?.addEventListener("click", () => {
      if (!set._reveal && !window.confirm("将显示密钥明文，操作会被记入审计日志（原型仅提示）。继续？")) return;
      set._reveal = !set._reveal;
      renderConfigDetailFiles(set, ver);
    });
  }
  ...
  function renderConfigDetailHistory(set) {
    ...
    $$("[data-cfg-hist-view]").forEach((b) => {
      b.addEventListener("click", () => {
        set._viewVersion = Number(b.dataset.cfgHistView);
        const tab = $(`#cfg-detail-tabs .tab[data-tab="files"]`);
        if (tab) tab.click();
        else renderConfigDetailFiles(set, findConfigVersion(set, set._viewVersion));
      });
    });
  ...
  function syncCreateConfigHelp() {
    const el = $("#create-config-default-path");
    if (el) el.textContent = CONFIG_MOUNT_PATH_PATTERN;
  }
    ...
    host.innerHTML = createConfigMounts
      .map((m) => {
        ...
        const setOpts =
          ...
          sets
            .map((s) => `<option value="${s.id}" ${s.id === m.setId ? "selected" : ""}>${escapeHtml(s.displayName)}（${escapeHtml(s.name)}）</option>`)
            .join("");
        const verOpts = set
          ? `<option value="latest_at_submit" ${m.pinPolicy === "latest_at_submit" ? "selected" : ""}>提交时最新（提交瞬间钉死，不会跟着改）</option>
             ...
             ${vers
               .map((v) => {
                 ...
                 return `<option value="${v.version}" ${m.pinPolicy === "pinned" && Number(m.version) === v.version ? "selected" : ""}>v${v.version} · ${escapeHtml(v.message || "")}${latestMark}</option>`;
               })
               .join("")}`
  ...
  const CREATE_DEFAULTS = {
    ...
    command:
      "torchrun --nnodes=${NNODES} --nproc_per_node=8 --rdzv_backend=c10d --rdzv_endpoint=${MASTER_ADDR}:29500 \\\n  pretrain_gpt.py --config configs/7b_phase4.yaml \\\n  --tensor-model-parallel-size 2 --pipeline-model-parallel-size 4 \\\n  --micro-batch-size 2 --global-batch-size 1024 --seq-length 8192 \\\n  --save /mnt/shared/checkpoints/slm-7b-phase4 \\\n  --load /mnt/shared/checkpoints/slm-7b-phase3 --save-interval 1000 --bf16",
    env: "NCCL_IB_HCA=mlx5\nCUDA_DEVICE_MAX_CONNECTIONS=1",
  };

  /** 工作目录 / Checkpoint 由平台按任务名约定落盘，用户无需填写 */
  function conventionPaths(taskName) {
    const slug = String(taskName || "unnamed")
      .trim()
      .replace(/[^a-zA-Z0-9._-]+/g, "-")
      .replace(/^-+|-+$/g, "") || "unnamed";
    return {
      workdir: `/mnt/shared/experiments/${slug}`,
      ckpt: `/mnt/shared/checkpoints/${slug}`,
    };
  }
  ...
  function switchCreateFormTab(tab) {
    ...
    $$("#create-form-panels > .tab-panel").forEach((p) => {
      p.classList.toggle("active", p.dataset.panel === tab);
    });
    const i = allowed.indexOf(tab);
    const prev = $("#btn-create-prev");
    const next = $("#btn-create-next");
    if (prev) prev.disabled = i <= 0;
    if (next) {
  ...
  function stepCreateFormTab(delta) {
    const tabs = visibleCreateFormTabs();
    const i = Math.max(0, Math.min(tabs.length - 1, tabs.indexOf(createFormTab) + delta));
    switchCreateFormTab(tabs[i]);
  }
  ...
  function syncCreateTabMeta() {
    ...
    setMeta("basic", basicDone ? name : "名称 / 优先级", basicDone);
    setMeta(
      "resources",
  ...
  function renderJobCreate() {
    ...
    if ($("#create-summary-title")) {
      $("#create-summary-title").textContent = isRerun ? "重跑预览" : "提交预览";
    }
    $("#btn-submit-job").textContent = isRerun ? "确认重跑" : "提交训练任务";

    const banner = $("#rerun-banner");
    const sourceTab = $("#create-tab-source");
    sourceTab?.classList.toggle("hidden", !(isRerun && source));
    if (isRerun && source) {
      ...
      $("#rerun-source-info").innerHTML = `
        <div class="kv-list">
          <div class="kv-row"><span class="k">任务 ID</span><span class="v mono">${source.id}</span></div>
          <div class="kv-row"><span class="k">原名称</span><span class="v">${source.name}</span></div>
          <div class="kv-row"><span class="k">团队 / 队列</span><span class="v">${escapeHtml(source.teamName || "—")} · ${escapeHtml(source.queue || "—")}</span></div>
          <div class="kv-row"><span class="k">优先级</span><span class="v">${renderJobPriorityBadge(source.priority)}</span></div>
          <div class="kv-row"><span class="k">数据中心</span><span class="v">${escapeHtml(dcName(source.dc))}</span></div>
          <div class="kv-row"><span class="k">队列 IB</span><span class="v"><span class="summary-yesno ${source.requireIB ? "is-yes" : "is-no"}">${source.requireIB ? "支持" : "不支持"}</span></span></div>
          <div class="kv-row"><span class="k">状态</span><span class="v">${badge(source.status)}</span></div>
          <div class="kv-row"><span class="k">资源</span><span class="v">${escapeHtml(jobResourceSummaryText(source))}</span></div>
          <div class="kv-row"><span class="k">镜像</span><span class="v mono" style="font-size:12px">${source.image}</span></div>
          <div class="kv-row"><span class="k">启动命令</span><span class="v mono" style="font-size:12px">${escapeHtml(source.command || "—")}</span></div>
          <div class="kv-row"><span class="k">创建时间</span><span class="v mono">${source.createdAt}</span></div>
          ${
            (source.configMounts || []).length
              ? `<div class="kv-row"><span class="k">原配置挂载</span><span class="v">${source.configMounts
                  .map((m) => `${escapeHtml(m.setName || m.setId)}@v${m.version}`)
                  .join("、")}</span></div>`
              : ""
          }
          ${
            source.failReason
              ? `<div class="kv-row"><span class="k">失败原因</span><span class="v text-danger">${escapeHtml(source.failReason)}</span></div>`
              : ""
          }
        </div>`;
    } else {
    ...
    if (isRerun && source) {
      ...
      fillCreateForm(CREATE_DEFAULTS);
    ...
    bindCodeEditor($("#create-command"), $("#create-command-hl"), "shell");
    bindCodeEditor($("#create-env"), $("#create-env-hl"), "env");
    switchCreateFormTab(createFormTab || "basic");
    updateCreateSummary();
  }
  ...
  function summaryFoldToggleHtml() {
    return `<div class="summary-fold-action" hidden>
      <button type="button" class="summary-fold-toggle" aria-expanded="false">
        <span class="summary-fold-toggle-label">展开</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg>
      </button>
    </div>`;
  }
  ...
  function summaryRestFold(title, bodyHtml) {
    return `<div class="summary-section summary-fold" data-fold="rest">
      <div class="summary-section-title">${title}</div>
      ${summaryFoldToggleHtml()}
      <div class="summary-fold-clip">${bodyHtml}</div>
    </div>`;
  }
  ...
  function summaryFoldMaxPx(el) {
    const raw = getComputedStyle(el).getPropertyValue("--summary-fold-max").trim();
    const n = parseFloat(raw);
    return Number.isFinite(n) ? n : 200;
  }

  function bindSummaryFolds(scope) {
    const root = scope || document;
    const apply = () => {
      $$(".summary-fold", root).forEach((el) => {
        const key = el.dataset.fold || "";
        const clip = el.querySelector(".summary-fold-clip");
        const btn = el.querySelector(".summary-fold-toggle");
        const action = el.querySelector(".summary-fold-action");
        if (!clip) return;

        const hasBody = !!clip.querySelector(".summary-cmd-body, .summary-fold-nested");
        const needs = hasBody || clip.scrollHeight > summaryFoldMaxPx(el) + 8;
        el.classList.toggle("is-collapsible", needs);
        const open = needs ? createSummaryFolds[key] === true : true;
        el.classList.toggle("is-open", open);

        if (action) action.hidden = !needs;
        if (!btn) return;
        btn.setAttribute("aria-expanded", open ? "true" : "false");
        const label = btn.querySelector(".summary-fold-toggle-label");
        if (label) label.textContent = open ? "收起" : "展开";
        btn.onclick = (e) => {
          e.preventDefault();
          const next = !el.classList.contains("is-open");
          if (key) createSummaryFolds[key] = next;
          el.classList.toggle("is-open", next);
          btn.setAttribute("aria-expanded", next ? "true" : "false");
          if (label) label.textContent = next ? "收起" : "展开";
        };
      });
    };
    apply();
  ...
  function updateCreateSummary() {
    ...
    const name = $("#create-name")?.value?.trim() || "—";
    const nodes = $("#create-nodes")?.value || "?";
    const gpn = $("#create-gpus-per-node")?.value || "?";
    const image = readCreateImage() || "—";
    const command = ($("#create-command")?.value || "").trim();
    const q = findQueue($("#create-queue")?.value);
    const team = findTeam($("#create-team")?.value);
    const gpuType = q?.gpuType || $("#create-gpu-type")?.value || "—";
    // IB 完全由队列功能特性决定
    const requireIB = q ? queueSupportsIB(q) : false;
    const priority = getCreatePriority();
    const pDef = jobPriorityDef(priority);
    const req = createJobResourceRequest();
    const total = req.totalGpus;
    const isRerun = Boolean(rerunFromJobId);
    const freeGpu = q ? queueGpuFree(q) : null;
    const freeCpu = q ? queueCpuFree(q) : null;
    const freeMem = q ? queueMemFree(q) : null;
    const overGpu = total != null && freeGpu != null && total > freeGpu;
    const overCpu = req.totalCpu != null && freeCpu != null && req.totalCpu > freeCpu;
    const overMem = req.totalMemGi != null && freeMem != null && req.totalMemGi > freeMem;
    const overQuota = overGpu || overCpu || overMem;
    const imageShort = String(image).split("/").pop() || "—";
    syncCreateCpuMemResetUi();

    summary.innerHTML = `
      <div class="summary-status">
        <span class="summary-status-badge ${isRerun ? "is-rerun" : "is-new"}">${isRerun ? "重跑" : "新建"}</span>
        ${
          overQuota
            ? `<span class="summary-status-badge is-queue-wait" title="申请 GPU 超过队列当前剩余额度">将排队</span>`
            : ""
        }
        ${isRerun ? `<span class="mono text-muted summary-status-src" title="${escapeHtml(rerunFromJobId || "")}">${escapeHtml(rerunFromJobId || "")}</span>` : ""}
      </div>

      ${summaryStaticSection(
        "基本信息",
        `<div class="summary-row"><span class="k">任务名称</span><span class="v" title="${escapeHtml(name)}">${escapeHtml(name)}</span></div>
        <div class="summary-row"><span class="k">团队</span><span class="v">${escapeHtml(team?.name || "—")}</span></div>
        <div class="summary-row"><span class="k">队列</span><span class="v" title="${escapeHtml(q?.displayName || q?.name || "")}">${escapeHtml(q?.displayName || q?.name || "—")}</span></div>
        <div class="summary-row"><span class="k">优先级</span><span class="v">${renderJobPriorityBadge(priority)} <span class="text-muted" style="font-size:11.5px;margin-left:4px">${escapeHtml(pDef.desc)}</span></span></div>
        <div class="summary-row"><span class="k">数据中心</span><span class="v">${q ? dcName(q.dc) : "—"}</span></div>
      ...
      ${summaryStaticSection(
        ...
        <div class="summary-row">
          <span class="k">总 GPU</span>
          <span class="v ${overGpu ? "is-warn" : ""}">
            ${total != null ? `${total} × ${escapeHtml(gpuType)}` : `? × ${escapeHtml(gpuType)}`}
            ${overGpu ? `<span class="summary-over-tip is-queue">将排队</span>` : ""}
          </span>
        </div>
        <div class="summary-row">
          <span class="k">CPU</span>
          <span class="v ${overCpu ? "is-warn" : ""}">${
            req.totalCpu != null
              ? `${req.totalCpu} 核（${req.cpuPerNode} 核/节点）`
              : "—"
          }${overCpu ? `<span class="summary-over-tip is-queue">将排队</span>` : ""}</span>
        </div>
        <div class="summary-row">
          <span class="k">内存</span>
          <span class="v ${overMem ? "is-warn" : ""}">${
            req.totalMemGi != null
              ? `${formatMemGi(req.totalMemGi)}（${req.memGiPerNode} GiB/节点）`
              : "—"
          }${overMem ? `<span class="summary-over-tip is-queue">将排队</span>` : ""}</span>
        </div>
        ${
          q
            ? `<div class="summary-row"><span class="k">队列剩余</span><span class="v mono">GPU ${freeGpu} 卡 · CPU ${freeCpu} 核 · ${formatMemGi(freeMem)}</span></div>`
            : ""
        }
        ${
          overQuota
            ? `<div class="summary-alert is-queue-wait" role="status">
                <strong>资源不足，提交后将排队</strong>
                <span>申请 GPU <strong>${total}</strong> 卡 / CPU <strong>${req.totalCpu ?? "—"}</strong> 核 / 内存 <strong>${req.totalMemGi != null ? formatMemGi(req.totalMemGi) : "—"}</strong>，队列当前剩余 GPU <strong>${freeGpu}</strong> 卡 · CPU <strong>${freeCpu}</strong> 核 · <strong>${formatMemGi(freeMem)}</strong>。任务可正常提交，进入队列等待资源释放后调度。</span>
              </div>`
            : ""
        }`
      )}
      ...
      ${summaryRestFold(
        ...
        ${summaryCommandBlockHtml(command)}
        <div class="summary-section summary-fold-nested">
          <div class="summary-section-title">配置挂载</div>
          ${createConfigMountSummaryHtml()}
        </div>`
      )}
    `;
    bindSummaryFolds(summary);
    syncCreateTabMeta();
  }

  function createConfigMountSummaryHtml() {
    const mounts = createConfigMounts.filter((m) => m.setId);
    const conflicts = createMountConflictMap();
    const hasConflict = mounts.some((m) => conflicts[m.uid]);
    if (!mounts.length) {
    ...
    const rows = mounts
      .map((m) => {
        const set = findConfigSet(m.setId);
        const files = selectedFilesForCreateMount(m);
        const verLabel =
          m.pinPolicy === "latest_at_submit" ? "提交时最新" : `v${m.version}`;
        const path = resolvedCreateMountPath(m);
        const bytes = files.reduce((s, f) => s + configFileBytes(f), 0);
        return `<div class="summary-row"><span class="k">${escapeHtml(set?.name || m.setId)}</span><span class="v">${escapeHtml(verLabel)}</span></div>
    ...
    return `${rows}${
      hasConflict
        ? `<div class="summary-alert is-warn" role="status"><strong>CONFIG_MOUNT_CONFLICT</strong><span>存在相等或互为前缀的挂载路径，提交前请修改。</span></div>`
        : ""
    }`;
  }
  ...
  function submitJob(e) {
    ...
    const newJob = {
      ...
      ...conventionPaths(name),
      pods: [],
      events: [
        {
          time: new Date().toTimeString().slice(0, 8),
          event: isRerun
            ? `由任务 ${rerunFromJobId} 重跑提交至队列 ${q.name}（${dcName(q.dc)} · ${netLabel} · ${prioLabel}）${queueNote}`
            : `任务已提交至队列 ${q.name}（${dcName(q.dc)} · ${netLabel} · 优先级 ${prioLabel}）${
  ...
  function renderJobDetail() {
    ...
    $("#job-detail-hero").innerHTML = `
      ...
      <div class="detail-meta">
        <div class="meta-item"><div class="label">创建人</div><div class="value">${escapeHtml(job.owner || "—")}</div></div>
        <div class="meta-item"><div class="label">优先级</div><div class="value">${renderJobPriorityBadge(job.priority)}</div></div>
        <div class="meta-item"><div class="label">创建时间</div><div class="value mono">${escapeHtml(job.createdAt || "—")}</div></div>
        <div class="meta-item"><div class="label">启动时间</div><div class="value mono">${escapeHtml(job.startedAt || "—")}</div></div>
        <div class="meta-item"><div class="label">运行时长</div><div class="value mono">${escapeHtml(job.duration || "—")}</div></div>
      </div>
    `;
  ...
  function renderPodsLogsSplit(job) {
        ...
        <div class="pod-log-pane pod-log-pane-right">
          <div class="pod-log-pane-head">
            <div style="min-width:0;flex:1">
              <div class="pod-log-pane-title">Pod 日志</div>
              <div class="mono text-muted" style="font-size:11.5px;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" id="selected-pod-label">
                ${selected.name} · ${selected.role} · ${selected.node}
              </div>
            </div>
    ...
    $$("[data-select-pod]").forEach((row) => {
      row.addEventListener("click", () => {
        ...
        if (label && pod) {
          label.textContent = `${pod.name} · ${pod.role} · ${pod.node}`;
        }
        renderLogLines(job);
      });
    });
  ...
  function jobPodsOf(job) {
    ...
    return [{ name: `${job?.name || "job"}-master-0`, role: "Master", node: "—", status: "Unknown", restarts: 0, gpu: "—" }];
  }
  ...
  function logsForPod(job, podName, { compact = false } = {}) {
    ...
    const isMaster = /master-0$/.test(podKey);
    ...
    return display.map((l) => ({ ...l, pod: podKey }));
  }
  ...
  function renderLogSearchTab(job) {
    ...
    el.innerHTML = `
      <div class="toolbar logsearch-toolbar">
        ...
        <button class="btn btn-primary btn-sm" id="btn-logsearch">搜索</button>
      </div>
  ...
  function renderConfigTab(job) {
    ...
    const gpn = gpusPerNodeOf(job);
    const imageShort = String(job.image || "—").split("/").pop() || "—";
    const imageFull = job.image || "—";
    $("#panel-config").innerHTML = `
      <div class="job-config-layout">
        <div class="job-config-main">
          <div class="summary-section">
            <div class="summary-section-title">任务与资源</div>
            <div class="kv-list">
              <div class="kv-row"><span class="k">任务名称</span><span class="v">${escapeHtml(job.name || "—")}</span></div>
              <div class="kv-row"><span class="k">团队</span><span class="v">${escapeHtml(job.teamName || "—")}</span></div>
              <div class="kv-row"><span class="k">队列</span><span class="v">${escapeHtml(q?.displayName || job.queue || "—")}</span></div>
              <div class="kv-row"><span class="k">优先级</span><span class="v">${renderJobPriorityBadge(job.priority)}</span></div>
              <div class="kv-row"><span class="k">数据中心</span><span class="v">${dcBadge(job.dc || q?.dc)}</span></div>
              <div class="kv-row"><span class="k">使用 IB</span><span class="v"><span class="summary-yesno ${useIB ? "is-yes" : "is-no"}">${useIB ? "是" : "否"}</span></span></div>
              <div class="kv-row"><span class="k">资源</span><span class="v job-res-kv">${renderJobResourceInline(job)}</span></div>
              ${
                job.rerunFrom
                  ? `<div class="kv-row"><span class="k">重跑自</span><span class="v mono link-cell" data-job="${escapeHtml(job.rerunFrom)}" style="cursor:pointer">${escapeHtml(job.rerunFrom)}</span></div>`
                  : ""
              }
            </div>
          </div>
          <div class="summary-section">
            <div class="summary-section-title">训练镜像</div>
            <div class="kv-list">
              <div class="kv-row"><span class="k">镜像</span><span class="v mono" title="${escapeHtml(imageFull)}">${escapeHtml(imageShort)}</span></div>
              <div class="kv-row"><span class="k">完整镜像</span><span class="v mono job-config-path">${escapeHtml(imageFull)}</span></div>
            </div>
          </div>
          <div class="summary-section">
            <div class="summary-section-title">配置挂载</div>
            <div class="kv-list">
              ${
                (job.configMounts || []).length
                  ? job.configMounts
                      .map(
                        (m) => `<div class="kv-row"><span class="k">${escapeHtml(m.displayName || m.setName || "配置集")}</span><span class="v">v${m.version} · <span class="mono">${escapeHtml(m.mountPath)}</span></span></div>`
                      )
                      .join("")
                  : `<div class="text-muted" style="padding:4px 0 0;font-size:12.5px">本次任务未通过平台挂载配置文件（启动命令中的路径来自镜像或共享盘）。</div>`
              }
            </div>
          </div>
        </div>
        <div class="job-config-side">
          <div class="summary-section-title">启动命令</div>
          ${codeBlockHtml(job.command || "# 无启动命令", "shell", "job-config-cmd-block")}
          <div class="summary-section-title mt-16">环境变量</div>
          ${codeBlockHtml(
            Object.keys(job.env || {}).length
          ...
          return `<div class="cfg-snapshot-card" data-snap-idx="${idx}">
            ...
            <div class="card-body flush">
              <table class="table cfg-snapshot-files">
                <thead><tr><th>路径</th><th>大小</th><th>类型</th><th class="th-actions">操作</th></tr></thead>
                <tbody>
                  ${files
                    .map(
                      (f, fi) => `<tr>
                        ...
                        <td>${f.kind === "secret" ? '<span class="badge cfg-kind-secret">密钥</span>' : '<span class="badge badge-info">普通</span>'}</td>
                      ...
                      <tr class="hidden" data-snap-body="${idx}:${fi}">
                        <td colspan="4">
                          ${
                            f.kind === "secret" && !m._reveal
                              ? codeBlockHtml("********", "text")
                              : codeBlockHtml(f.content || "", langFromPath(f.path))
                          }
                          ${
                            f.kind === "secret"
                              ? `<button type="button" class="btn btn-secondary btn-sm mt-8" data-snap-reveal="${idx}">${m._reveal ? "隐藏明文" : "显示明文"}</button>`
                              : ""
                          }
                        </td>
                      </tr>`
  ...
  function bindJobConfigSnapshots(job) {
    ...
    $$("[data-snap-reveal]").forEach((b) => {
      b.addEventListener("click", () => {
        const m = (job.configMounts || [])[+b.dataset.snapReveal];
        if (!m) return;
        if (!m._reveal && !window.confirm("将显示该任务快照中的密钥明文，操作记入审计（原型仅提示）。继续？")) return;
        m._reveal = !m._reveal;
        renderConfigTab(job);
      });
    });
  ...
  function renderStorage() {
    $("#panel-storage").innerHTML = `
      ...
      <div class="card mt-16">
        ...
        <div class="card-body">
          ${codeBlockHtml(
            `# 数据中心本地数据（HostPath，随任务数据中心选择）
/data/jiangsu      # 疆算
/data/liangjiang   # 重庆两江
/data/shuitu       # 重庆水土

# 可选全局共享
/mnt/shared        # JuiceFS 共享工作区：代码、实验输出
/mnt/ckpt          # 高速 Checkpoint（可选）

# 调度：Queue.dc → nodeSelector maip.io/datacenter → HostPath 挂载对应路径`,
            "path"
          )}
        </div>
      </div>`;
  ...
  function renderDcMgmt() {
    ...
    const enabled = list.filter((d) => d.enabled !== false).length;
    const disabled = list.length - enabled;
    const regions = new Set(
      list.map((d) => d.region).filter((r) => r && r !== "—")
    ).size;
    const def = defaultDc();

    const kpi = $("#dc-mgmt-kpi");
    if (kpi) {
      ...
      kpi.innerHTML = `
        ...
        <div class="stat-card" style="--stat-color: var(--text-3)">
          <div class="stat-label">已停用</div>
          <div class="stat-value" style="color:var(--text-3)">${disabled}</div>
        </div>
        <div class="stat-card" style="--stat-color: var(--purple)">
          <div class="stat-label">默认数据中心</div>
          <div class="stat-value" style="font-size:20px">${escapeHtml(def.short || def.name || "默认")}</div>
        </div>`;
    }

    const hint = $("#dc-list-hint");
    if (hint) {
      hint.textContent = `内置默认数据中心始终可用 · 标识用于节点 / 队列 / 集群关联 · Label Key 固定为 maip.io/datacenter`;
    }

    const search = $("#dc-mgmt-search");
    if (search && search.value !== (dcMgmtFilter.q || "")) search.value = dcMgmtFilter.q || "";
    const st = $("#dc-mgmt-filter-status");
    if (st) st.value = dcMgmtFilter.status || "all";
  ...
  function renderDcMgmtList() {
    ...
    if (!body) return;
    let list = [...(MOCK.datacenters || [])];
    if (dcMgmtFilter.status === "enabled") list = list.filter((d) => d.enabled !== false);
    if (dcMgmtFilter.status === "disabled") list = list.filter((d) => d.enabled === false);
    if (dcMgmtFilter.q) {
      const q = dcMgmtFilter.q.toLowerCase();
      list = list.filter(
        (d) =>
    ...
    body.innerHTML = pageList.length
      ? `<div class="table-wrap"><table class="table">
        <thead><tr>
          <th>数据中心</th><th>标识</th><th>区域</th><th>Label</th><th>关联</th><th>状态</th><th class="th-actions">操作</th>
        </tr></thead>
        <tbody>
          ${pageList
            .map((d) => {
            ...
            <tr class="${enabled ? "" : "is-disabled-row"}">
              <td>
                <div style="display:flex;align-items:center;gap:8px">
                  ...
                  <div>
                    ...
                    ${isDef ? ` <span class="badge badge-info">默认</span>` : ""}
              ...
              <td class="td-actions">
                <div class="job-actions">
                  ...
                  ${
                    isDef
                      ? `<button type="button" class="btn btn-secondary btn-sm" disabled title="默认数据中心始终启用">停用</button>
                         <button type="button" class="btn btn-danger btn-sm" disabled title="默认数据中心不可删除">删除</button>`
                      : `${
                          enabled
                            ? `<button type="button" class="btn btn-secondary btn-sm" data-dc-disable="${escapeHtml(d.id)}">停用</button>`
                            : `<button type="button" class="btn btn-primary btn-sm" data-dc-enable="${escapeHtml(d.id)}">启用</button>`
                        }
                  <button type="button" class="btn btn-danger btn-sm" data-dc-delete="${escapeHtml(d.id)}">删除</button>`
                  }
                </div>
              </td>
            </tr>`;
    ...
    $$("[data-dc-enable]").forEach((b) =>
      b.addEventListener("click", () => openDcAction("enable", b.dataset.dcEnable))
    );
    $$("[data-dc-disable]").forEach((b) =>
      b.addEventListener("click", () => openDcAction("disable", b.dataset.dcDisable))
    );
  ...
  function openDcForm(dcId) {
    ...
    set("#dc-form-color", color);
    set("#dc-form-color-text", color);
  ...
  function submitDcForm() {
    ...
    let color = ($("#dc-form-color-text")?.value || $("#dc-form-color")?.value || "").trim();
    ...
    if (!editingDcId) {
      if (id === "default") {
        showErr("标识 default 为系统保留，请使用其它标识");
        return;
      }
  ...
  function openDcAction(action, dcId) {
    ...
    if (d.isDefault && (action === "delete" || action === "disable")) {
      toast(
        action === "delete" ? "默认数据中心不可删除" : "默认数据中心始终启用，不可停用",
        "warning"
      );
      return;
    }
    ...
    const defName = defaultDc()?.name || "默认数据中心";
    const titleMap = {
      enable: "确认启用数据中心",
      disable: "确认停用数据中心",
      delete: "确认删除数据中心",
    ...
    const msgMap = {
      enable: `确定要启用数据中心 <strong>${escapeHtml(name)}</strong> 吗？`,
      disable: `确定要停用数据中心 <strong>${escapeHtml(name)}</strong> 吗？`,
      delete: `确定要删除数据中心 <strong>${escapeHtml(name)}</strong> 吗？`,
    };
    const hintMap = {
      enable: "启用后，新建队列等场景可再次选择该数据中心。",
      disable: "停用后，新建队列等场景将不可再选择该数据中心；已有节点 / 队列关联不受影响。",
      delete:
        usage.total > 0
          ? `当前关联 ${usage.nodes} 节点 / ${usage.queues} 队列 / ${usage.clusters} 集群。删除后这些资源将自动改挂到「${defName}」。`
          : "删除后不可恢复。",
    };
    const confirmCls = action === "enable" ? "btn btn-primary" : "btn btn-danger";
    const confirmText =
      action === "enable" ? "确认启用" : action === "disable" ? "确认停用" : "确认删除";
    ...
    if (metaEl) {
      metaEl.textContent = [
        d.id,
        d.label || buildDcLabel(d.labelKey, d.id),
  ...
  function confirmDcAction() {
    ...
    if (pending.action === "enable") {
      d.enabled = true;
      d.updatedAt = "2026-07-27 " + new Date().toTimeString().slice(0, 8);
      closeDcAction();
      toast(`已启用数据中心 ${d.name}`);
    } else if (pending.action === "disable") {
      if (d.isDefault) {
        closeDcAction();
        toast("默认数据中心始终启用，不可停用", "warning");
        return;
      }
      d.enabled = false;
      d.updatedAt = "2026-07-27 " + new Date().toTimeString().slice(0, 8);
      closeDcAction();
      toast(`已停用数据中心 ${d.name}`, "warning");
    } else if (pending.action === "delete") {
      if (d.isDefault) {
        closeDcAction();
        toast("默认数据中心不可删除", "warning");
        return;
      }
      const removedId = d.id;
      const removedName = d.name;
      const fallbackId = defaultDcId();
      // 关联资源迁移到默认数据中心，避免删除后业务数据悬空
      (MOCK.nodes || []).forEach((n) => {
        if (n.dc === removedId) {
          n.dc = fallbackId;
          if (n.labels && typeof n.labels === "object") {
            n.labels["maip.io/datacenter"] = fallbackId;
          }
          recordNodeMaintOp({
            action: "set-dc",
            node: n.name,
            remark: `删除数据中心「${removedName}」后迁移至默认数据中心 ${formatDcLabel(fallbackId)}`,
          });
        }
      });
      (MOCK.queues || []).forEach((q) => {
        if (q.dc === removedId) q.dc = fallbackId;
      });
      (MOCK.jobs || []).forEach((j) => {
        if (j.dc === removedId) j.dc = fallbackId;
      });
      (MOCK.clusterCapacity || []).forEach((c) => {
        if (c.dc === removedId) c.dc = fallbackId;
      });
      (MOCK.pools || []).forEach((p) => {
        if (p.dc === removedId) p.dc = fallbackId;
      });
      (MOCK.clusters || []).forEach((c) => {
        if (Array.isArray(c.dcs) && c.dcs.includes(removedId)) {
          c.dcs = c.dcs.filter((id) => id !== removedId);
          if (!c.dcs.includes(fallbackId)) c.dcs.push(fallbackId);
        }
      });
      MOCK.datacenters = (MOCK.datacenters || []).filter((x) => x.id !== removedId);
      ensureDefaultDatacenter();
      closeDcAction();
      toast(`已删除数据中心 ${removedName}，关联资源已迁移至默认数据中心`, "warning");
    } else {
      closeDcAction();
    }
    renderDcMgmt();
  ...
  function ensureNodeMgmtFilters() {
    ...
    if (dcSel) {
      ...
      // 筛选项：全部已启用数据中心 + 节点上出现过的数据中心 + 未分配
      const enabledDcs = (MOCK.datacenters || []).filter((d) => d.enabled !== false);
      const seenIds = new Set(clusterNodes.map((n) => nodeDcId(n)).filter(Boolean));
      const extra = (MOCK.datacenters || []).filter(
        (d) => seenIds.has(d.id) && !enabledDcs.some((e) => e.id === d.id)
      );
      const dcs = [...enabledDcs, ...extra];
      dcSel.innerHTML =
    ...
    if (nodeMgmtFilter.q) {
      ...
      list = list.filter((n) => {
        ...
        return (
          ...
          dcName(n.dc).toLowerCase().includes(q) ||
          isolateInfoOf(n).toLowerCase().includes(q) ||
          k8sNodeKubectlStatus(n).toLowerCase().includes(q) ||
          labelStr.includes(q)
        );
      });
    }
  ...
  function openNodeDcForm(nodeNames) {
    ...
    if (sel) {
      const dcs = (MOCK.datacenters || []).filter((d) => d.enabled !== false);
      // 若仅单节点且已有数据中心，预选当前值
      let preferred = "";
          ...
          .map(
            (d) =>
              `<option value="${escapeHtml(d.id)}" ${d.id === preferred ? "selected" : ""}>${escapeHtml(
                d.name
              )}${d.isDefault ? "（默认）" : ""} · ${escapeHtml(d.id)}</option>`
          )
          .join("");
    }
  ...
  function submitNodeDcForm() {
    ...
    if (dc.enabled === false) {
      if (err) {
        err.textContent = "该数据中心已停用，请选择其他数据中心";
        err.style.display = "";
      }
      return;
    }
        ...
        <div class="node-detail-panels" id="node-detail-panels">
          <div class="tab-panel active" data-panel="overview" role="tabpanel">
            <div class="kv-grid">
              ...
              <div class="kv-item"><span class="k">Cordon</span><span class="v">${
                cordoned
                  ? '<span class="badge badge-warning">已封锁</span>'
                  : '<span class="badge badge-healthy">未封锁</span>'
              }</span></div>
  ...
  function renderNodeLabelsEditorUI(opts = {}) {
    ...
    el.innerHTML = entries.length
      ? entries
        ...
        <div class="node-label-row" data-label-idx="${idx}">
          <input type="text" class="node-label-k" value="${escapeHtml(k)}" spellcheck="false" />
          <input type="text" class="node-label-v" value="${escapeHtml(v ?? "")}" placeholder="(empty)" spellcheck="false" />
    ...
    $$("[data-label-del-idx]").forEach((b) =>
      b.addEventListener("click", () => {
        ...
        const list = Object.entries(collectNodeLabelsDraftFromUI());
        if (idx < 0 || idx >= list.length) return;
        list.splice(idx, 1);
        pendingNodeLabelsDraft = Object.fromEntries(list);
  ...
  function collectNodeLabelsDraftFromUI() {
    ...
    $$("#node-labels-editor .node-label-row").forEach((row) => {
      const k = (row.querySelector(".node-label-k")?.value || "").trim();
      const v = row.querySelector(".node-label-v")?.value ?? "";
      if (k) draft[k] = v;
    });
    return draft;
  }
  ...
  function addNodeLabelRow() {
    ...
    const value = $("#node-label-value")?.value ?? "";
    const err = $("#node-labels-error");
    if (!key) {
      if (err) {
        err.textContent = "请输入标签 key";
        err.style.display = "";
      }
      return;
    }
    if (existing.some(([k]) => k === key)) {
      if (err) {
        err.textContent = `标签 ${key} 已存在，请直接修改或先删除`;
        err.style.display = "";
      }
      return;
    }
    ...
    if ($("#node-label-key")) $("#node-label-key").value = "";
    if ($("#node-label-value")) $("#node-label-value").value = "";
    if (err) {
      err.style.display = "none";
      err.textContent = "";
    }
  ...
  function submitNodeLabels() {
    ...
    if (mode === "merge" && !Object.keys(draft).length) {
      if (err) {
        err.textContent = "请至少添加一个要批量写入的标签";
        err.style.display = "";
      }
      return;
    }
  ...
  function openNodeTaintsEditor(nodeNameOrNames) {
    ...
    if ($("#node-taint-key")) $("#node-taint-key").value = "";
    if ($("#node-taint-value")) $("#node-taint-value").value = "";
    if ($("#node-taint-effect")) $("#node-taint-effect").value = "NoSchedule";
    renderNodeTaintsEditorUI();
  ...
  function addNodeTaintRow() {
    ...
    const effect = $("#node-taint-effect")?.value || "NoSchedule";
    const err = $("#node-taints-error");
    if (!key) {
      if (err) {
        err.textContent = "请输入污点 key";
        err.style.display = "";
      }
      return;
    }
    ...
    if (exists) {
      if (err) {
        err.textContent = "相同 key + effect 的污点已存在";
        err.style.display = "";
      }
      return;
    }
  ...
  function submitNodeTaints() {
    ...
    if (mode === "merge" && !pendingNodeTaintsDraft.length) {
      if (err) {
        err.textContent = "请至少添加一个要批量写入的污点";
        err.style.display = "";
      }
      return;
    }
  ...
  function renderQueueMgmtList() {
    ...
    body.innerHTML = pageList.length
      ? `<div class="table-wrap"><table class="table">
        <thead><tr>
          <th>队列</th><th>团队</th><th>数据中心</th><th>GPU</th><th>额度（已用 / 总量）</th><th>功能特性</th><th>状态</th><th class="th-actions">操作</th>
        </tr></thead>
        <tbody>
          ${pageList
            .map((q) => {
  ...
  /** 队列启用 / 禁用 / 删除二次确认 */
  let pendingQueueAction = null; // { action: enable|disable|delete, queueId }
  ...
  function openQueueActionConfirm(action, queueId) {
    ...
    const activeCount = activeJobsForQueue(q.id).length;
    const titleMap = {
      disable: "确认禁用队列",
      enable: "确认启用队列",
      delete: "确认删除队列",
    ...
    const hintMap = {
      disable: "禁用后，新建任务将不可再选择该队列；已在运行 / 排队的任务不受影响。可随时重新启用。",
      enable: "启用后，该队列将重新出现在新建任务的队列选择列表中，团队成员可再次提交任务。",
      delete: activeCount
        ? `该队列仍有 ${activeCount} 个运行 / 排队中的任务。删除后新任务不可再选择该队列，团队关联将解除；已有任务记录会保留队列名称。此操作不可撤销。`
  ...
  function confirmQueueAction() {
    ...
    if (pending.action === "disable") {
      q.state = "Closed";
      closeQueueActionConfirm();
      toast(`已禁用队列「${q.displayName || q.name}」，新任务将不可选择该队列`, "warning");
      renderQueueMgmtList();
      return;
    }
    if (pending.action === "enable") {
    ...
    if (dcSel) {
      ...
      dcSel.innerHTML =
        ...
        (MOCK.datacenters || [])
          .map(
            (d) =>
              `<option value="${d.id}" ${cur === d.id ? "selected" : ""}>${escapeHtml(d.name)}${
                d.enabled === false ? "（停用）" : ""
              }</option>`
          )
          .join("");
  ...
  function setQueueTeamSelection(team) {
    const hiddenName = $("#q-form-team");
    const hiddenId = $("#q-form-team-id");
    const search = $("#q-form-team-search");
    const chip = $("#q-team-selected");
    const clearBtn = $("#q-team-clear");
    const results = $("#q-team-results");
    if (!team) {
    ...
    if (hiddenName) hiddenName.value = team.name || "";
    if (hiddenId) hiddenId.value = team.id || "";
    if (search) {
    ...
    if (chip) {
      ...
      const queueCount = (team.queueIds || []).length;
      chip.innerHTML = `
        <span class="user-picker-chip">
          <span class="user-picker-chip-avatar">${escapeHtml((team.name || "?").slice(0, 1))}</span>
          <span class="user-picker-chip-meta">
            <strong>${escapeHtml(team.name || "")}</strong>
            <span class="text-muted">${queueCount} 个关联队列</span>
            ${team.owner ? `<span class="text-muted">负责人 ${escapeHtml(team.owner)}</span>` : ""}
          </span>
        </span>`;
    }
  ...
  function resolveTeamForQueue(q) {
    if (!q) return null;
    const teams = MOCK.teams || [];
    if (q.teamId) {
      const byId = teams.find((t) => t.id === q.teamId);
      if (byId) return byId;
    }
    if (q.team) {
      const byName = teams.find((t) => t.name === q.team);
      if (byName) return byName;
      // 兼容历史短名（SLM / 研究组 等）
      const ql = String(q.team).toLowerCase();
      return (
        teams.find(
          (t) =>
            (t.name || "").toLowerCase().includes(ql) ||
            ql.includes((t.name || "").slice(0, 2).toLowerCase())
        ) || null
      );
    }
    return null;
  ...
  function renderQueueTeamResults(q) {
    ...
    if (!box) return;
    if ($("#q-form-team-search")?.readOnly) {
      box.classList.add("hidden");
      return;
    }
    const ql = (q || "").trim().toLowerCase();
    let list = (MOCK.teams || []).filter((t) => t.status !== "disabled");
    if (ql) {
      list = list.filter(
        (t) =>
          (t.name || "").toLowerCase().includes(ql) ||
    ...
    if (!list.length) {
      box.innerHTML = `<div class="user-picker-empty">无匹配团队，请先在「团队管理」中创建</div>`;
    ...
    box.innerHTML = list
      ...
      .join("");
    box.classList.remove("hidden");
    $$("[data-q-team-id]", box).forEach((btn) => {
      btn.addEventListener("click", () => {
        const team = (MOCK.teams || []).find((x) => x.id === btn.dataset.qTeamId);
        if (team) {
          setQueueTeamSelection(team);
  ...
  function openQueueForm(editId) {
    ...
    if (dcSel) {
      ensureDefaultDatacenter();
      // 新建仅可选启用数据中心；编辑时保留当前数据中心即使已停用
      const dcs = (MOCK.datacenters || []).filter(
        (d) => d.enabled !== false || (q && d.id === q.dc)
      );
      dcSel.innerHTML = dcs.length
        ? dcs
            .map(
              (d) =>
                `<option value="${d.id}">${escapeHtml(d.name)}（${escapeHtml(d.id)}）${
                  d.isDefault ? " · 默认" : ""
                }${d.enabled === false ? " · 已停用" : ""}</option>`
            )
            .join("")
        : `<option value="${defaultDcId()}">默认数据中心</option>`;
    }
    $("#q-form-name").value = q?.name || "";
    $("#q-form-name").disabled = Boolean(q);
    ...
    if (dcSel) {
      ...
      if (!preferred) {
        ...
        const firstWithCap = (MOCK.datacenters || []).find(
          (d) => d.enabled !== false && capDcs.has(d.id)
        );
        preferred = firstWithCap?.id || defaultDcId();
      }
    ...
    // 所属团队：搜索下拉
    setQueueTeamSelection(resolveTeamForQueue(q));
  ...
  function submitQueueForm() {
    ...
    const teamName = $("#q-form-team")?.value?.trim() || "";
    const teamId = $("#q-form-team-id")?.value?.trim() || "";
    const features = readQueueFormFeatures();
    ...
    if (!teamName && !teamId) {
      if (err) {
        err.textContent = "请从下拉中选择所属团队";
        err.classList.remove("hidden");
      }
      $("#q-form-team-search")?.focus();
      return;
    }
    if (!editId && (MOCK.queues || []).some((q) => q.name === name)) {
      if (err) {
        err.textContent = "队列标识已存在";
        err.classList.remove("hidden");
      }
      return;
    }
  ...
  /** 将队列 ID 写入所属团队的 queueIds（并从其他团队移除） */
  function syncTeamQueueLink(queueId, teamId) {
    if (!queueId) return;
    (MOCK.teams || []).forEach((t) => {
      t.queueIds = (t.queueIds || []).filter((id) => id !== queueId);
    });
    if (!teamId) return;
    const team = (MOCK.teams || []).find((t) => t.id === teamId);
    if (team) {
      if (!Array.isArray(team.queueIds)) team.queueIds = [];
      if (!team.queueIds.includes(queueId)) team.queueIds.push(queueId);
    }
  }
  ...
  function renderUserMgmt() {
    ...
    body.innerHTML = `
              ...
              pageList.length
                ...
                <tr class="${checked ? "is-row-selected" : ""}">
                  ...
                  <td class="td-actions">
                    ...
                    .join("")
                : `<tr><td colspan="10"><div class="empty-state">暂无平台用户，请点击「从 LDAP 添加」</div></td></tr>`
            }
          </tbody>
        </table>
      </div>
      ...
      <div class="text-muted" style="padding:10px 16px;font-size:12px">
        平台用户 ${ (MOCK.ldapUsers || []).length } 人 · LDAP 目录约 ${ (MOCK.ldapDirectory || []).length } 人 ·
        配置 ${cfg.enabled ? "已启用" : "已停用"} · ${cfg.host || "未配置"}:${cfg.port || "—"}
      </div>`;
  ...
  function renderLdapConfigPanel() {
    const cfg = MOCK.ldapConfig || {};
    const statusEl = $("#ldap-config-status");
    const formEl = $("#ldap-config-form");
    if (!formEl) return;

    const testOk = cfg.lastTestResult === "success";
    if (statusEl) {
      statusEl.innerHTML = `
        <div class="stats-grid stats-grid-4">
          <div class="stat-card" style="--stat-color: ${cfg.enabled ? "var(--success)" : "var(--warning)"}">
            <div class="stat-label">LDAP 开关</div>
            <div class="stat-value" style="font-size:20px">${cfg.enabled ? "已启用" : "已停用"}</div>
            <div class="stat-meta">${escapeHtml(cfg.name || "—")}</div>
          </div>
          <div class="stat-card" style="--stat-color: ${testOk ? "var(--success)" : "var(--danger)"}">
            <div class="stat-label">最近测试</div>
            <div class="stat-value" style="font-size:18px">${testOk ? "成功" : cfg.lastTestResult === "fail" ? "失败" : "未测"}</div>
            <div class="stat-meta mono">${cfg.lastTestAt || "—"}</div>
          </div>
          <div class="stat-card" style="--stat-color: var(--primary)">
            <div class="stat-label">服务地址</div>
            <div class="stat-value mono" style="font-size:16px">${escapeHtml(cfg.host || "—")}:${cfg.port || "—"}</div>
            <div class="stat-meta">${cfg.useTls ? "LDAPS / TLS" : "明文 LDAP"}</div>
          </div>
          <div class="stat-card" style="--stat-color: var(--purple)">
            <div class="stat-label">平台用户 / 目录</div>
            <div class="stat-value" style="font-size:20px">${(MOCK.ldapUsers || []).length}<span class="stat-value-unit"> / ${(MOCK.ldapDirectory || []).length}</span></div>
            <div class="stat-meta">可用用户 · LDAP 目录缓存</div>
          </div>
        </div>
        ${
          cfg.lastTestMessage
            ? `<div class="card mt-16"><div class="card-body" style="font-size:12.5px;line-height:1.55">
                <strong>最近测试结果</strong>
                <div class="${testOk ? "text-success" : "text-danger"}" style="margin-top:6px">${escapeHtml(cfg.lastTestMessage)}</div>
              </div></div>`
            : ""
        }`;
    }
          ...
          <div class="form-grid">
            <div class="form-group full">
              <label class="checkbox-inline">
                <input type="checkbox" id="ldap-enabled" ${cfg.enabled ? "checked" : ""} /> 启用 LDAP 认证与目录检索
              </label>
            </div>
            <div class="form-group full">
            ...
            <div class="form-group full">
              ...
              <input id="ldap-bind-pwd" type="password" value="${escapeHtml(cfg.bindPassword || "")}" placeholder="留空则不修改" />
            </div>
            <div class="form-group">
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="flex gap-8 mt-16">
            <button type="button" class="btn btn-secondary" id="btn-ldap-goto-users">去用户管理添加用户</button>
          </div>
        </div>
      </div>`;

    $("#btn-ldap-goto-users")?.addEventListener("click", () => navigate("user-mgmt"));
  ...
  function readLdapConfigForm() {
    ...
    return {
      ...cfg,
      enabled: Boolean($("#ldap-enabled")?.checked),
  ...
  function testLdapConfig() {
    ...
    // 原型：模拟测试结果
    const ok = Boolean(next.enabled && next.host && next.bindDn);
    Object.assign(MOCK.ldapConfig, next, {
  ...
  function openUserLdapModal() {
    const cfg = MOCK.ldapConfig || {};
    if (!cfg.enabled) {
      toast("LDAP 未启用，请先在系统配置 · LDAP 中启用", "warning");
      navigate("system-config", { tab: "ldap" });
      return;
    }
    ldapAddSelected = new Set();
    const cfgBox = $("#modal-user-ldap-cfg");
    if (cfgBox) {
      cfgBox.innerHTML = `
        <div class="ldap-add-cfg-inner">
          <span class="tag">${cfg.useTls ? "LDAPS" : "LDAP"}</span>
          <span class="mono" style="font-size:12px">${escapeHtml(cfg.host)}:${cfg.port}</span>
          <span class="text-muted" style="font-size:12px">Base ${escapeHtml(cfg.baseDn || "—")}</span>
          ${
            cfg.lastTestResult === "success"
              ? '<span class="badge badge-healthy">最近测试成功</span>'
              : '<span class="badge badge-warning">建议先测试连接</span>'
          }
        </div>`;
    }
  ...
  function renderTeamMgmt() {
    ...
    detailEl.innerHTML = `
      <div class="rp-detail-head">
        <div>
          <h3 style="font-size:16px;color:var(--text-0);margin:0">${p.name}</h3>
      ...
      <div class="form-section-title mt-16" style="display:flex;justify-content:space-between;align-items:center">
        ...
        <button type="button" class="btn btn-secondary btn-sm" id="btn-team-add-member">+ 添加成员</button>
      </div>
      ...
      <div class="form-section-title mt-16" style="display:flex;justify-content:space-between;align-items:center">
        ...
        <button type="button" class="btn btn-primary btn-sm" id="btn-team-link-queue">管理队列</button>
      </div>
      ${
        queues.length
  ...
  function openTeamForm() {
    ...
    $("#modal-team")?.classList.add("show");
    setTimeout(() => $("#team-form-name")?.focus(), 40);
  }

  function closeTeamForm() {
    $("#modal-team")?.classList.remove("show");
  ...
  function submitTeamForm() {
    ...
    if ((MOCK.teams || []).some((p) => p.name === name)) {
      if (err) {
    ...
    const ownerUser = activePlatformUsers().find((u) => u.name === ownerName);
    ...
    const team = {
      id,
      name,
      desc: $("#team-form-desc")?.value?.trim() || "",
      owner,
      members: [owner],
      queueIds: [],
    ...
    if (!(ownerUser.teamIds || []).includes(id)) {
      ownerUser.teamIds = [...(ownerUser.teamIds || []), id];
    }
    currentTeamId = id;
    teamMgmtFilter.page = 1;
    teamMgmtFilter.q = "";
  ...
  function init() {
    initTheme();
    initSidebarCollapse();
    // 保证默认数据中心始终存在，任务 / 节点 / 队列无需额外配置即可关联
    ensureDefaultDatacenter();
    initClusterSelect();
    ...
    $("#btn-create-prev")?.addEventListener("click", () => stepCreateFormTab(-1));
    $("#btn-create-next")?.addEventListener("click", () => stepCreateFormTab(1));
    $$("#create-form-tabs [data-create-tab]").forEach((el) => {
      el.addEventListener("click", () => switchCreateFormTab(el.dataset.createTab));
    });
    ...
    $("#dc-mgmt-search")?.addEventListener("input", (e) => {
      dcMgmtFilter.q = e.target.value || "";
      dcMgmtFilter.page = 1;
      renderDcMgmtList();
    });
    $("#dc-mgmt-filter-status")?.addEventListener("change", (e) => {
      dcMgmtFilter.status = e.target.value || "all";
      dcMgmtFilter.page = 1;
      renderDcMgmtList();
    });
    ...
    $("#dc-form-color")?.addEventListener("input", (e) => {
      const v = e.target.value;
      const t = $("#dc-form-color-text");
      if (t) t.value = v;
    });
    $("#dc-form-color-text")?.addEventListener("input", (e) => {
      const v = (e.target.value || "").trim();
      if (/^#[0-9A-Fa-f]{6}$/.test(v)) {
        const c = $("#dc-form-color");
        if (c) c.value = v;
      }
    });
    ...
    $("#q-form-team-search")?.addEventListener("focus", () => {
      if (!$("#q-form-team-search")?.readOnly) renderQueueTeamResults($("#q-form-team-search")?.value || "");
    });
    $("#q-form-team-search")?.addEventListener("input", (e) => {
      if ($("#q-form-team-search")?.readOnly) return;
      // 输入时清除已选隐藏值，要求从列表点选
      const hiddenName = $("#q-form-team");
      const hiddenId = $("#q-form-team-id");
      if (hiddenName) hiddenName.value = "";
      if (hiddenId) hiddenId.value = "";
      $("#q-team-selected")?.classList.add("hidden");
      $("#q-team-clear")?.classList.add("hidden");
      renderQueueTeamResults(e.target.value || "");
    });
    $("#q-team-clear")?.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      setQueueTeamSelection(null);
      renderQueueTeamResults("");
      $("#q-form-team-search")?.focus();
    });
    document.addEventListener("click", (e) => {
