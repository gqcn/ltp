...
2. 随着公司`SLM`预训练项目推进，训练规模和集群规模明显扩大，现有资源包括 15 张`B300`、30+ 张`H100`、128 张`H100`、16 张`H200`等多个集群。训练平台需要在一个`SLM`集群上先完成`MVP`替换落地，并逐步对比`AI-HPC`平台，在监控告警覆盖度、故障自愈、实验管理、研发效率和平台可扩展性方面形成优势。
...
- 训推融合不是只看某一个调度器的单点能力，而是统一用户、项目、配额、资源账本、镜像、观测、审计和`API`。如果训练和推理都能稳定运行在`K8S`资源池中，平台收益更高；如果训练侧存在短期不可接受的性能或稳定性风险，再按工作负载类型拆分为`Slurm`训练池和`K8S`开发机/推理池。


## 3.2 底层核心调度器方案选型

底层核心调度器决定训练任务如何获得`GPU`、节点、网络拓扑、存储和运行时环境。但训练平台选型不能只看调度器本身，还必须同时评估周边工具建设、运维成本、监控告警、可观测性、权限审计、资源额度分配、镜像和日志体系、平台`API`集成、后续扩展等完整建设成本。
...
| 周边工具建设成本 | 开发机、镜像、日志、实验、监控、告警、资源账本、审计、故障诊断、任务模板等是否有成熟生态可复用。 | 高。平台短板主要在产品化和工程化能力。 |
...
| 资源额度和配额管理 | 是否支持项目、用户、队列、`GPU`、存储、开发机和训练任务的统一配额、审计和资源账本。 | 高。平台需要支撑多团队、多项目共享训练资源。 |
...
| 调度与训练执行 | 使用`Volcano`等批调度组件承载训练，具备`Gang Scheduling`、队列、配额、异构设备和训练框架插件能力；开发、训练、推理资源更容易统一。 |
...
| 运维和资源治理 | 运维复杂度中等：保留现有`Slurm`运维体系，同时新增`K8S`平台服务运维。资源治理需要统一用户、项目、配额、账本和审计，但可逐步建设，不要求首期重构训练资源池。 |
...
- 平台短板主要在开发机、门户、任务`API`、日志诊断、实验管理、镜像管理、资源配额和自动化入口，这些能力更适合由`K8S`和云原生生态统一承载。
- 公司推理平台已经使用`K8S + Volcano`，如果训练平台也采用同一技术底座，训练和推理在镜像、观测、权限、资源队列、运维工具和平台`API`上可以复用更多能力，整体技术栈反而更简单。
- `K8S + Slurm`混合方案训练侧风险更低，但会长期引入两套调度域、两套运维知识、两套状态和日志映射、两套资源配额治理逻辑。除非`K8S`训练`POC`无法达标，否则不应作为默认目标态。
- 资源额度分配需要平台统一建模用户、项目、队列、开发机、训练任务、`GPU`、存储和审计记录。纯`K8S`路径优先映射到`Volcano Queue`；混合兜底路径再同步映射到`Slurm account/QoS/TRES`。

因此，本方案最终选择“纯`K8S`优先，`K8S + Slurm`兜底”的路线：

- `MVP`主线优先采用`K8S + Volcano`作为训练执行后端，同时由`K8S`承载开发机、平台控制面、日志、实验入口、镜像管理和监控告警组件。
- 在`MVP`正式展开前设置训练闭环`POC`作为准入门槛，只有当`K8S + Volcano`通过核心训练场景验证后，才进入纯`K8S`主线实施。
- 平台层仍建设统一的`SchedulerBackend`、任务状态模型、日志模型、资源账本、用户项目权限和审计模型，避免上层入口绑定具体调度器。
...
该设计可以最大化复用推理平台已有`K8S + Volcano`经验，让训练、推理、开发机和平台服务在镜像、日志、监控、权限、队列、资源账本和运维工具上更统一。`Slurm Backend`保留为兜底后端：如果`K8S`训练`POC`未通过，可以切换为`Slurm`承载训练执行，并继续复用同一套`CLI`、任务模型、状态模型、日志接口和权限体系。

## 3.4 为什么优先采用纯`K8S`架构

`Slurm`在`HPC`和大规模训练领域非常成熟，尤其在高性能网络、拓扑感知调度、批任务资源管理方面有明显优势。

但从整个平台能力建设看，训练平台不仅包含训练调度，还包含开发机、镜像管理、任务页面、日志聚合、实验管理、权限隔离、资源配额、自动化`API`和`AI Agent`调用等平台化能力。如果训练执行也能在`K8S + Volcano`上跑通，纯`K8S`可以显著降低长期技术栈复杂度，训练和推理也能在同一套云原生资源治理体系下演进。
...
- 公司推理平台已使用`K8S + Volcano`，训练平台可以复用既有`Volcano Queue`、批调度、资源隔离和运维经验。
- 平台不需要长期维护`Slurm`和`K8S`两套状态机、日志路径、资源账本和权限映射。
...
- `Volcano`队列、配额、优先级和`Gang Scheduling`验证通过。
...
### 3.4.5 未来`AI Agent`自动化调用需要稳定`API`
...
### 3.4.6 结论：纯`K8S`优先，`Slurm`兜底

本方案建议：

- `MVP`主线优先采用纯`K8S`架构。
- `K8S`负责开发机、平台控制面和训练任务执行。
- `Volcano`作为训练批调度组件，承载队列、配额、`Gang Scheduling`和训练任务调度。
...
flowchart TB
  ...
  api --> workspaceSvc["DevWorkspaceService"]
  api --> jobSvc["TrainingJobService"]
  api --> imageSvc["ImageService"]
  api --> expSvc["ExperimentService"]
  api --> logSvc["LogService"]
  ...
  k8sPool --> workspace["JupyterLab 开发机 Pod"]
...
- **`MVP`功能克制**：优先打通预训练核心闭环，不在第一阶段引入复杂工作流和后训练全链路。
...
| 训练任务列表 | 任务状态、提交人、项目、镜像、资源、运行时长。 | 支持按项目、用户、状态筛选任务，快速查看任务运行概况。 |
| 训练任务详情 | 任务规格、运行实例、事件、状态流转、失败原因。 | 支持查看任务配置、调度信息、执行状态和失败诊断信息。 |
| 训练日志 | 角色、实例、时间范围、关键字。 | 支持按任务、角色和实例查看日志，并支持关键字检索。 |
| 镜像管理 | 可用镜像、标签、用途、基础环境、扫描状态。 | 支持查看平台可用镜像及其适用场景，辅助开发机和训练任务选择镜像。 |
| 实验入口 | `SwanLab local`实验看板访问入口。 | 支持从训练任务跳转或代理访问对应实验看板。 |
...
| 业务模块 | 主要职责 |
| --- | --- |
| **用户管理** | 管理平台用户身份、登录态、用户基础信息和外部账号映射。 |
| **项目管理** | 管理项目空间、项目成员、项目资源归属和任务归属。 |
| **权限管理** | 校验用户在项目、开发机、训练任务、镜像和实验等资源上的操作权限。 |
| **配额管理** | 管理项目和用户维度的`CPU`、内存、`GPU`、存储、开发机数量和训练任务额度。 |
| **开发机管理** | 管理开发机创建、启动、停止、状态查询、访问入口和空闲回收。 |
| **训练任务管理** | 管理训练任务创建、查询、取消、状态聚合、任务元数据和运行记录。 |
| **镜像管理** | 管理镜像列表、镜像元数据、镜像标签、适用场景和镜像可用性校验。 |
| **日志管理** | 提供任务日志查询、日志流式输出、关键字检索和日志定位能力。 |
| **实验管理** | 管理`SwanLab`实验路径、看板入口、实验与训练任务关联关系。 |
...
| `MVP`选择 | 优先适配`K8S + Volcano`，在正式实施前通过训练闭环`POC`验证多机多卡训练、`NCCL`/`RDMA`性能、队列配额和故障恢复能力。 |
...
| 主要风险 | 多后端保留会带来状态映射、日志关联、资源账本和故障诊断的一致性治理成本，需要在实施阶段专项设计。 |
...
| 字段 | 含义 | 说明 |
| --- | --- | --- |
| `metadata.project` | 项目归属 | 用于权限、配额和任务归档。 |
...
- `Service`：分布式训练服务发现。
- `PodGroup`/队列配置：表达`Gang Scheduling`、队列、优先级和资源配额。
...
SWANLAB_PROJ_NAME={project}
SWANLAB_EXP_NAME={job_name}
SWANLAB_RUN_ID={job_id}
```

训练镜像中预置`swanlab`包，训练代码可通过`swanlab.init()`写入实验记录。平台将`job_id`、用户、项目、`SwanLab`日志目录建立关联。
...
本方案不建议把`Determined AI`或其他单一开源训练平台作为整体二次开发基础。当前业务核心诉求同时包含`K8S + Volcano`训练执行、`K8S`容器化开发机、统一`CLI`、平台门户、镜像管理、任务日志、实验管理、权限配额和`Slurm`兜底扩展。现有开源项目通常只在其中一两个方向成熟，强行选一个大平台二开会引入架构迁就和长期维护风险。
...
## 10.4 本方案采用或优先集成的开源组件

| 组件 | 使用位置 | 选型说明 |
| --- | --- | --- |
| `Kubernetes` | 平台控制面、开发机资源池和训练执行底座 | 承载`Web Portal`、`Platform API`、`JupyterLab`开发机`Pod`、训练`Pod`、反向代理、日志和实验看板等平台服务。 |
| `Volcano` | 优先训练批调度组件 | 作为`K8S`训练任务队列、`Gang Scheduling`、配额和公平共享能力的主路径。公司推理平台已采用`Volcano`并运行相对成熟稳定，因此本方案`K8S`训练后端只考虑`Volcano`，不引入`Kueue`。 |
...
| `DCGM Exporter`/`kube-state-metrics` | `GPU`和`K8S`资源指标 | 采集`GPU`利用率、显存、温度、功耗和`K8S`对象状态。 |
...
`MVP`页面不需要替代`Grafana`，但需要在训练任务详情中展示关键状态和跳转链接。
...
- 统一沿用公司推理平台已采用的`Volcano + Queue`方案管理资源队列和额度。
- 平台将租户、项目和资源额度元数据下发为`Volcano Queue`配置，用于表达`CPU`、内存、`GPU`等资源额度、队列优先级和公平共享策略。
- 开发机、`K8S`训练任务和推理任务在提交时关联对应`Volcano Queue`，避免绕过平台额度体系。
- `Namespace`仅作为对象隔离、权限边界或部署组织方式，不作为资源额度管理方案。
- 使用`NetworkPolicy`限制不必要访问。

`Slurm`兜底层面建议：

- 按项目、团队或业务域配置`account`、`QoS`、`TRES`和`FairShare`等资源策略。
...
- 启用`Slurm`兜底时，`Slurm`作业提交应通过平台受控服务账号或受控网关执行，避免用户绕过平台配额和审计。
...
本次评审先聚焦`P0`能力，下方`MVP`功能清单为从完整功能清单中拷贝出的`P0`功能集合；其后的完整功能清单仍保留`P1/P2`作为后续扩展参考。

## 13.1 `MVP`功能清单

| 模块 | 功能 | 优先级 | 描述 |
| --- | --- | :---: | --- |
| 开发环境管理 | `JupyterLab`开发机 | P0 | 提供基于`JupyterLab`的在线开发环境，用户可以通过浏览器访问开发机进行代码编辑、小样本调试、依赖验证和任务提交准备。 |
| 开发环境管理 | 开发机生命周期管理 | P0 | 支持开发机创建、启动、停止、重建、状态查看和异常清理，`MVP`阶段可按用户限制一个活跃开发机实例，降低资源占用和调度复杂度。 |
| 开发环境管理 | 开发机资源规格 | P0 | 支持选择预置`CPU`、内存、`GPU`规格和镜像，平台将规格转换为`K8S Pod`资源请求和限制，并接入项目或用户配额。 |
| 开发环境管理 | 开发环境持久化 | P0 | 开发机挂载共享存储，用户代码、配置、依赖缓存、实验文件和中间数据保存在`/data/home/{username}`等持久化目录中，开发机停止或重建后数据不丢失。 |
| 开发环境管理 | 访问入口与鉴权 | P0 | 平台提供开发机访问入口，通过`Ingress`或反向代理进入用户对应的`JupyterLab`实例，并在产品化阶段叠加平台登录态、访问审计和链接有效期控制。 |
| 开发辅助能力 | 数据集挂载 | P0 | 将共享存储或数据集目录挂载到开发机和训练任务中，保证开发调试和正式训练可以读取一致的数据路径。 |
| 开发辅助能力 | 开发调试 | P0 | 支持模型代码运行、小样本调试、日志查看和基础问题定位，帮助用户在提交正式训练前完成快速验证。 |
| 训练任务管理 | 统一训练任务规格 | P0 | 定义与底层后端无关的`TrainingJobSpec`，统一描述镜像、命令、资源、存储、环境变量、实验配置、队列、优先级和拓扑约束。 |
| 训练任务管理 | `aitrain CLI` | P0 | 提供训练任务命令行工具，支持登录、任务校验、`dry-run`、提交、查询、日志查看和取消任务；支持`--output json`，便于自动化系统和`AI Agent`调用。 |
| 训练任务管理 | 训练任务提交 | P0 | `MVP`阶段优先通过`CLI`提交正式训练任务，平台将任务规格转换为`K8S + Volcano`训练任务，并记录平台任务元数据。 |
| 训练任务管理 | 单机训练 | P0 | 支持单节点单卡或单节点多卡训练任务，用于小模型、小样本调试和基础训练流程验证。 |
| 训练任务管理 | 分布式训练 | P0 | 支持多节点、多`GPU`协同训练，优先覆盖`PyTorch DDP`类预训练任务，并通过`NCCL`/`RDMA`验证通信性能。 |
| 训练任务管理 | 训练任务列表 | P0 | 提供训练任务列表页面，支持按项目、用户、状态、时间、任务名称等维度筛选，展示任务状态、资源规格、运行时长、镜像和提交人。 |
| 训练任务管理 | 训练任务详情 | P0 | 提供任务详情页，展示任务规格、调度状态、运行实例、节点分布、事件、日志入口、失败原因、输出目录和实验记录入口。 |
| 训练任务管理 | 任务取消 | P0 | 支持用户或项目管理员取消运行中或排队中的训练任务，平台同步更新任务状态，并清理或保留相关运行资源和日志记录。 |
| 训练配置管理 | 资源配置 | P0 | 支持为训练任务指定节点数、单节点`GPU`数、`CPU`、内存等资源规格，适配不同规模训练任务。 |
| 训练配置管理 | 环境变量和参数配置 | P0 | 支持为训练任务配置环境变量、启动参数和模型超参数，实现参数化训练和任务复现。 |
| 调度与资源管理 | `K8S + Volcano`训练后端 | P0 | 以`K8S + Volcano`作为`MVP`优先训练执行后端，支持将训练任务渲染为`Volcano Job`，承载队列、配额、优先级和`Gang Scheduling`能力。 |
| 调度与资源管理 | 训练闭环`POC` | P0 | 在`MVP`正式实施前完成`K8S`训练闭环验证，覆盖单机单卡、单机多卡、多机多卡`PyTorch DDP`、`NCCL`/`RDMA`性能、共享存储和故障诊断。 |
| 调度与资源管理 | 队列与配额 | P0 | 沿用公司推理平台已有`Volcano Queue`思路，按项目或团队管理`CPU`、内存、`GPU`等资源额度，训练任务和开发机提交时关联对应队列。 |
| 调度与资源管理 | 调度后端抽象 | P0 | 平台通过`SchedulerBackend`屏蔽`K8S + Volcano`、`Slurm`等底层差异，上层`Web`、`CLI`、`AI Agent`使用统一任务模型和状态模型。 |
| 存储与目录管理 | 共享存储挂载 | P0 | 开发机和训练任务统一挂载共享存储，保证代码、数据、日志、`checkpoint`和实验记录在开发调试和正式训练中使用一致路径。 |
| 存储与目录管理 | 用户目录规范 | P0 | 约定`/data/home/{username}`作为用户个人持久化目录，`/data/home/{username}/workspace`作为工作目录，`/data/home/{username}/outputs/{job_id}`作为任务输出目录。 |
| 存储与目录管理 | 训练输出管理 | P0 | 训练任务输出目录按`job_id`组织，保存日志、`checkpoint`、实验记录和导出产物，方便用户在开发机和任务详情页中追踪。 |
| 镜像管理 | 镜像列表 | P0 | 对接公司既有`Harbor`，展示可用于开发机和训练任务的镜像列表、标签、大小、更新时间和基础环境信息。 |
| 镜像管理 | 镜像选择 | P0 | 开发机创建和训练任务提交时支持选择平台允许的镜像，平台校验镜像权限和可用性，避免用户使用不合规镜像。 |
| 镜像管理 | 预置镜像 | P0 | 平台提供开发机、训练、微调可用的标准基础镜像，预置常用框架、`CUDA`、`PyTorch`和调试工具，做到开箱即用。 |
| 日志与诊断 | 训练日志查看 | P0 | 支持按任务、角色、实例查看训练日志，提供`tail`、关键字检索和基础日志定位能力；`K8S + Volcano`后端优先读取`Pod logs`。 |
| 日志与诊断 | 调度事件查看 | P0 | 在任务详情页展示`Volcano Job`、`Pod`、调度事件、镜像拉取状态、节点分配、退出码和失败原因，降低用户排障成本。 |
| 实验管理 | `SwanLab local`集成 | P0 | 训练任务启动时注入`SwanLab local`相关环境变量，将实验日志写入任务输出目录，并与平台`job_id`建立关联。 |
| 监控告警 | 平台服务监控 | P0 | 监控`Web Portal`、`Platform API`、数据库、任务适配器等平台组件状态，保障平台控制面可用。 |
| 监控告警 | 训练资源监控 | P0 | 通过`Prometheus`、`DCGM Exporter`、`kube-state-metrics`等采集节点、`GPU`、训练`Pod`、`Volcano Job`和队列资源使用情况。 |
| 权限与项目管理 | 用户与项目 | P0 | 支持按用户和项目组织开发机、训练任务、镜像、实验和资源配额，用户只能访问自己或项目授权的资源。 |
| 资源治理 | 资源配额 | P0 | 为项目、团队或用户设置`CPU`、内存、`GPU`、存储、开发机数量和训练任务额度上限，防止资源滥用。 |
| 平台门户 | 任务与开发机视图 | P0 | `Web`侧优先提供查看、诊断和跳转能力，复杂训练任务创建暂不在`MVP`阶段建设，通过`CLI`完成。 |

为便于评审和分阶段交付，完整功能清单按栏目拆分为开发入口、训练执行、资产与存储、观测与实验、治理与扩展五类。
...
| 开发环境管理 | 开发环境持久化 | P0 | 开发机挂载共享存储，用户代码、配置、依赖缓存、实验文件和中间数据保存在`/data/home/{username}`等持久化目录中，开发机停止或重建后数据不丢失。 |
| 开发环境管理 | 访问入口与鉴权 | P0 | 平台提供开发机访问入口，通过`Ingress`或反向代理进入用户对应的`JupyterLab`实例，并在产品化阶段叠加平台登录态、访问审计和链接有效期控制。 |
| 开发环境管理 | 自动回收 | P1 | 支持按闲置时长自动停止或回收开发机，释放长期空闲的`CPU`、内存和`GPU`资源，降低开发资源浪费。 |
| 开发环境管理 | 多实例管理 | P2 | 支持用户创建、切换、停止多个开发机实例，用于不同项目或不同任务并行开发；`MVP`主线可先限制单活实例，后续扩展多实例。 |
| 开发环境管理 | 协作共享 | P2 | 支持开发环境、代码目录或实验产物在项目成员之间共享，方便团队协同调试模型；`MVP`阶段先通过共享存储和项目权限预留能力。 |
| 开发辅助能力 | `Web Terminal` | P1 | 在浏览器中提供命令行终端，方便用户执行系统命令、启动脚本、查看文件和进行环境排查。 |
| 开发辅助能力 | `Git`版本管理 | P1 | 支持在开发机中使用`Git`拉取、提交、切换分支和管理训练代码版本，保障代码来源可追溯。 |
| 开发辅助能力 | 数据集挂载 | P0 | 将共享存储或数据集目录挂载到开发机和训练任务中，保证开发调试和正式训练可以读取一致的数据路径。 |
| 开发辅助能力 | 开发调试 | P0 | 支持模型代码运行、小样本调试、日志查看和基础问题定位，帮助用户在提交正式训练前完成快速验证。 |
| 开发辅助能力 | 依赖管理 | P1 | 支持用户在开发机中通过`pip`、`conda`等方式安装和验证依赖，并将依赖固化到项目镜像或任务环境中。 |
...
| 训练任务管理 | 训练任务详情 | P0 | 提供任务详情页，展示任务规格、调度状态、运行实例、节点分布、事件、日志入口、失败原因、输出目录和实验记录入口。 |
| 训练任务管理 | 任务取消 | P0 | 支持用户或项目管理员取消运行中或排队中的训练任务，平台同步更新任务状态，并清理或保留相关运行资源和日志记录。 |
| 训练任务管理 | 训练任务模板 | P1 | 支持将常用训练配置沉淀为模板，复用镜像、资源、环境变量、命令和存储路径，减少重复任务创建成本。 |
| 训练配置管理 | 资源配置 | P0 | 支持为训练任务指定节点数、单节点`GPU`数、`CPU`、内存等资源规格，适配不同规模训练任务。 |
| 训练配置管理 | 环境变量和参数配置 | P0 | 支持为训练任务配置环境变量、启动参数和模型超参数，实现参数化训练和任务复现。 |
| 训练配置管理 | 网络配置 | P1 | 支持表达`RDMA`、`NCCL`、节点拓扑等高性能网络相关配置，满足多机多卡训练的通信性能要求。 |
| 调度与资源管理 | `K8S + Volcano`训练后端 | P0 | 以`K8S + Volcano`作为`MVP`优先训练执行后端，支持将训练任务渲染为`Volcano Job`，承载队列、配额、优先级和`Gang Scheduling`能力。 |
| 调度与资源管理 | 训练闭环`POC` | P0 | 在`MVP`正式实施前完成`K8S`训练闭环验证，覆盖单机单卡、单机多卡、多机多卡`PyTorch DDP`、`NCCL`/`RDMA`性能、共享存储和故障诊断。 |
| 调度与资源管理 | 队列与配额 | P0 | 沿用公司推理平台已有`Volcano Queue`思路，按项目或团队管理`CPU`、内存、`GPU`等资源额度，训练任务和开发机提交时关联对应队列。 |
| 调度与资源管理 | 调度后端抽象 | P0 | 平台通过`SchedulerBackend`屏蔽`K8S + Volcano`、`Slurm`等底层差异，上层`Web`、`CLI`、`AI Agent`使用统一任务模型和状态模型。 |
| 调度与资源管理 | `Slurm`兜底预留 | P1 | `MVP`主线不默认引入`Slurm`，但保留`Slurm Backend`设计位置；当`K8S + Volcano`训练`POC`无法满足性能、稳定性或交付周期要求时启用。 |
...
| 存储与目录管理 | 用户目录规范 | P0 | 约定`/data/home/{username}`作为用户个人持久化目录，`/data/home/{username}/workspace`作为工作目录，`/data/home/{username}/outputs/{job_id}`作为任务输出目录。 |
| 存储与目录管理 | 训练输出管理 | P0 | 训练任务输出目录按`job_id`组织，保存日志、`checkpoint`、实验记录和导出产物，方便用户在开发机和任务详情页中追踪。 |
| 存储与目录管理 | 权限隔离 | P1 | `MVP`阶段按用户`UID/GID`和目录权限隔离个人目录；公共目录按项目或共享目录管理，后续补充容量配额、`ACL`和审计策略。 |
| 物料管理 | 代码仓库对接 | P1 | 支持从`Git`代码仓库拉取训练代码，记录代码版本信息，为训练复现和问题追踪提供依据。 |
| 物料管理 | 模型文件归档 | P1 | 存储训练完成后的模型权重文件和导出产物，并与训练任务、实验记录和输出目录建立关联。 |
| 物料管理 | 训练代码归档 | P2 | 归档训练过程中使用的代码版本、启动脚本和关键配置，便于后续复现实验结果。 |
| 物料管理 | 辅助资料管理 | P2 | 支持保存实验报告、说明文档、评估结果等训练相关辅助资料，作为任务产物的一部分进行管理。 |
| 镜像管理 | 镜像列表 | P0 | 对接公司既有`Harbor`，展示可用于开发机和训练任务的镜像列表、标签、大小、更新时间和基础环境信息。 |
| 镜像管理 | 镜像元数据 | P1 | 维护镜像用途、框架版本、`CUDA`版本、`PyTorch`版本、`NCCL`版本、适用场景和项目可见范围，辅助用户选择合适镜像。 |
| 镜像管理 | 镜像选择 | P0 | 开发机创建和训练任务提交时支持选择平台允许的镜像，平台校验镜像权限和可用性，避免用户使用不合规镜像。 |
| 镜像管理 | 预置镜像 | P0 | 平台提供开发机、训练、微调可用的标准基础镜像，预置常用框架、`CUDA`、`PyTorch`和调试工具，做到开箱即用。 |
| 镜像管理 | 公共镜像 | P1 | 提供所有项目或租户可见的通用镜像，适用于通用开发、调试和训练场景。 |
| 镜像管理 | 私有镜像 | P1 | 支持项目或租户专属镜像，保障自定义环境、依赖和代码资产的隔离与安全。 |
| 镜像管理 | 镜像版本管理 | P1 | 对镜像进行版本标记、更新时间展示和历史版本追踪，避免基础镜像更新导致环境不可复现。 |
| 镜像管理 | 镜像扫描 | P2 | 对镜像漏洞、恶意程序和安全合规风险进行扫描，作为企业安全准入和后续治理能力。 |
| 镜像管理 | 镜像构建 | P2 | 预留自动化构建和在线构建能力，后续支持从代码仓库或配置页面触发自定义镜像构建。 |

## 13.5 日志诊断、训练可视化与实验记录

| 模块 | 功能 | 优先级 | 描述 |
| --- | --- | :---: | --- |
| 日志与诊断 | 训练日志查看 | P0 | 支持按任务、角色、实例查看训练日志，提供`tail`、关键字检索和基础日志定位能力；`K8S + Volcano`后端优先读取`Pod logs`。 |
| 日志与诊断 | 调度事件查看 | P0 | 在任务详情页展示`Volcano Job`、`Pod`、调度事件、镜像拉取状态、节点分配、退出码和失败原因，降低用户排障成本。 |
| 日志与诊断 | 开发机日志与状态 | P1 | 展示开发机`Pod`状态、重启次数、失败原因、资源使用和访问状态，辅助定位开发环境启动失败或运行异常。 |
| 日志与诊断 | 日志聚合预留 | P2 | `MVP`阶段可以先通过`K8S API`和文件路径读取日志，中期接入`Loki`或`ELK`，支持长周期保存、检索和告警。 |
| 训练可视化 | 训练进度 | P1 | 展示训练任务的运行进度、运行时长、`step`/`epoch`等基础进度信息，帮助用户判断任务是否正常推进。 |
| 训练可视化 | 训练指标 | P1 | 展示`Loss`、`Accuracy`等关键训练指标曲线，`MVP`阶段优先通过`SwanLab local`提供。 |
| 训练可视化 | 告警通知 | P1 | 当训练失败、资源异常、任务完成或存储挂载异常时，通过平台告警或消息渠道通知用户。 |
| 实验管理 | `SwanLab local`集成 | P0 | 训练任务启动时注入`SwanLab local`相关环境变量，将实验日志写入任务输出目录，并与平台`job_id`建立关联。 |
| 实验管理 | 实验入口 | P1 | 任务详情页展示实验记录路径和看板入口，支持用户从任务跳转到对应实验记录，`MVP`阶段可通过`swanlab watch`或开发机查看。 |
| 实验管理 | 任务与实验关联 | P1 | 平台记录任务、用户、项目、输出目录和实验目录的映射关系，为后续实验对比、指标聚合和模型归档打基础。 |
| 实验管理 | 实验对比 | P2 | 支持多个实验之间的参数、指标和产物对比，帮助用户选择效果更优的训练配置。 |
| 实验管理 | 实验复现 | P2 | 基于任务规格、代码版本、镜像版本、参数和数据路径复现历史训练任务。 |
| 实验管理 | 实验分组 | P2 | 支持按项目、模型、任务类型或实验主题对实验进行分组管理。 |
| 监控告警 | 平台服务监控 | P0 | 监控`Web Portal`、`Platform API`、数据库、任务适配器等平台组件状态，保障平台控制面可用。 |
| 监控告警 | 训练资源监控 | P0 | 通过`Prometheus`、`DCGM Exporter`、`kube-state-metrics`等采集节点、`GPU`、训练`Pod`、`Volcano Job`和队列资源使用情况。 |
| 监控告警 | 基础告警 | P1 | 覆盖平台服务不可用、开发机启动失败、训练任务异常失败、`GPU`异常、存储挂载失败、队列资源不足等关键场景。 |
...
| --- | --- | :---: | --- |
| 权限与项目管理 | 用户与项目 | P0 | 支持按用户和项目组织开发机、训练任务、镜像、实验和资源配额，用户只能访问自己或项目授权的资源。 |
| 权限与项目管理 | 角色权限 | P1 | `MVP`阶段至少区分普通用户、项目管理员和平台管理员，控制任务查看、取消、开发机管理和项目资源查看权限。 |
| 权限与项目管理 | 操作审计 | P1 | 记录开发机创建/停止、训练任务提交/取消、镜像选择、任务状态变更等关键操作，为后续审计和问题追踪提供依据。 |
| 权限与项目管理 | 资源权限 | P1 | 控制用户对计算、存储、网络、镜像等资源的访问和使用权限，避免跨项目或跨租户越权使用。 |
| 资源治理 | 资源配额 | P0 | 为项目、团队或用户设置`CPU`、内存、`GPU`、存储、开发机数量和训练任务额度上限，防止资源滥用。 |
| 资源治理 | 后计费策略 | P2 | 预留按实际资源使用量进行统计、结算和成本分摊的能力，作为后续平台运营治理方向。 |
| 平台门户 | 首页与导航 | P1 | 提供面向算法同学的统一入口，能快速进入开发机、训练任务、镜像、日志和实验相关页面。 |
| 平台门户 | 任务与开发机视图 | P0 | `Web`侧优先提供查看、诊断和跳转能力，复杂训练任务创建暂不在`MVP`阶段建设，通过`CLI`完成。 |
| 流水线能力 | 流水线编排 | P2 | 预留将数据预处理、训练、评估、导出等步骤编排为自动化工作流的能力；`MVP`阶段不引入复杂`DAG`主链路。 |
| 流水线能力 | 流水线模板 | P2 | 支持保存标准化流程配置，便于团队复用最佳实践。 |
| 流水线能力 | 流水线监控与日志 | P2 | 支持查看流水线各步骤状态和日志，作为后续后训练和评测流水线建设方向。 |
