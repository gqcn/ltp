...
2. 随着公司`SLM`预训练项目推进，训练规模和集群规模明显扩大，现有资源包括 15 台`B300`、30\+ 台`H100`、16 台`H200`等多个集群。训练平台需要在一个`SLM`集群上先完成`MVP`替换落地，并逐步对比`AI-HPC`平台，在监控告警覆盖度、故障自愈、实验管理、研发效率和平台可扩展性方面形成优势。

3. 当前算法用户反馈的最大痛点集中在**训练稳定性**和**可观测性**。典型问题是`GPU`卡、`IB`端口等硬件异常无法在平台侧提前发现和处理，训练任务失败后，算法同学需要从复杂日志中人工定位故障，再手动下线故障机器、调整训练规模参数和`checkpoint`恢复策略后继续训练，严重影响预训练效率。因此，`MVP`阶段的首要优先级不是堆叠完整平台功能，而是先解决故障提前发现、故障资源调度剔除、资源修复后重新入池、训练过程指标实时观测和异常告警。
...
|**`NVIDIA`**|`Run:ai`和开源`KAI Scheduler`明确面向`Kubernetes`上的`AI`工作负载和`GPU`调度。`DGX`/`Base Command Manager`类`HPC/AI`集群管理产品可与不同工作负载管理系统组合，公开资料不等价于“训练只用`K8S`”。|`Grove`面向`Kubernetes`上的拓扑优化推理编排，`Dynamo`、`Triton`、`TensorRT-LLM`等更偏推理运行时和服务框架。<br>|`NVIDIA`在云原生`AI`调度和推理编排上明显走`K8S`路线，但`HPC`训练集群仍允许按部署选择`Slurm`等后端。|[NVIDIA Run:ai Documentation](https://run-ai-docs.nvidia.com/)、[KAI Scheduler](https://github.com/kai-scheduler/KAI-Scheduler)、[Grove](https://github.com/ai-dynamo/grove)、[Dynamo](https://github.com/ai-dynamo/dynamo)、[Triton Inference Server](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/)、[TensorRT\-LLM](https://nvidia.github.io/TensorRT-LLM/)|
...
|**字节/火山引擎**|公开云产品和云原生生态更多体现`K8S`、批调度、弹性资源池和自研平台能力；内部大模型训练是否统一使用`Slurm`或`K8S`没有完整公开口径。|推理侧更接近在线服务和云原生资源管理形态，通常不会使用`Slurm`作为在线推理服务调度器。|可参考其云原生调度、资源池和训推资源协同思路，但不能简单推断为独立`K8S`或独立`Slurm`。|[火山引擎机器学习平台](https://www.volcengine.com/product/ml_platform)、[火山引擎容器服务 VKE](https://www.volcengine.com/product/vke)|
|**阿里云/阿里**|公开论文和产品资料显示`PAI`是自研`AI`平台体系，云产品侧更偏托管训练和云原生资源管理；未公开表述为统一使用`Slurm`。|`PAI-EAS`、`RTP-LLM`等属于托管推理服务和自研推理引擎方向，不是`Slurm`型批任务调度。|更接近自研平台、云原生/托管服务路线，不能作为独立`Slurm`路线依据。|[阿里云人工智能平台 PAI](https://help.aliyun.com/zh/pai/)、[DLC 分布式训练任务](https://help.aliyun.com/zh/pai/user-guide/submit-jobs-by-using-the-console)、[EAS 模型在线服务](https://help.aliyun.com/zh/pai/user-guide/model-service-deployment-by-using-the-pai-console)、[RTP\-LLM](https://github.com/alibaba/rtp-llm)|
...
结合现有`SLM`集群、既有`AI-HPC`迁移路径、公司推理平台已使用`K8S + Volcano`、以及`MVP`需要优先支撑大规模预训练的约束，本方案重点比较四类方案：独立`Slurm`、独立`K8S`、`K8S + Slurm`混合架构、`Slurm on K8S`。
...
|**运维和资源治理**|运维团队可复用既有经验，训练侧风险最低；但独立`Slurm`无法自然承载`Web Portal`、开发机、镜像、实验看板等云原生服务，周边服务还要另找承载底座。|
...
|**综合判断**|适合作为`K8S`训练`POC`失败或短期无法达标时的兜底方案。它训练侧风险更低，但平台技术栈更复杂，长期收益低于独立`K8S`。|
...
- 多租户、项目管理和资源配额不进入`MVP`交付范围，但平台需要统一建模用户、默认队列、开发机、训练任务、`GPU`、存储和审计记录，为后续项目空间、配额、资源账本和成本分摊扩展保留接口。独立`K8S`路径优先预留到`Volcano Queue`；混合兜底路径再预留到`Slurm account/QoS/TRES`。

因此，本方案最终选择“**独立****`K8S`****优先，****`K8S + Slurm`****兜底**”的路线：

- `MVP`主线优先采用`K8S + Volcano`作为训练执行后端，同时由`K8S`承载开发机、平台控制面、日志、实验入口、镜像管理、资源健康检测、故障隔离和监控告警组件。

- 在`MVP`正式展开前设置训练闭环`POC`作为准入门槛，只有当`K8S + Volcano`通过核心训练场景验证后，才进入独立`K8S`主线实施。
...
本方案采用独立`K8S`优先路线。`K8S`负责容器化开发机、平台控制面和训练任务执行，训练调度优先使用公司已有经验的`Volcano`。`CLI`、`Web`页面、`AI Agent`等上层入口不直接绑定`kubectl`、`Volcano`或`Slurm`，而是统一调用`Platform API`。`Platform API`内部通过`SchedulerBackend`接口适配不同底层调度系统。
...
但从整个平台能力建设看，训练平台不仅包含训练调度，还包含开发机、镜像管理、任务页面、日志聚合、实验管理、权限隔离、资源健康和监控告警等平台化能力。如果训练执行也能在`K8S + Volcano`上跑通，独立`K8S`方案可以显著降低长期技术栈复杂度，训练和推理也能在同一套云原生资源治理体系下演进。

因此，本方案将独立`K8S`作为优先目标架构，`K8S + Slurm`作为训练闭环验证失败后的兜底方案。

### 独立`K8S`优先的收益

独立`K8S`路线的主要收益不是某个单点调度能力更强，而是整体平台复杂度更低：
...
**当前独立****`K8S`****路线最大的风险，是公司还没有在****`K8S`****上走通过完整训练流程。因此，技术选型不能直接跳到实施阶段，必须先做训练闭环****`POC`****。**`POC`至少覆盖：
...
  k8sPool --> volcanoJob["Volcano<br/>训练任务"]
...
- **独立****`K8S`****优先**：`K8S`承载`JupyterLab`开发机、平台服务和`MVP`阶段训练执行，`Volcano`作为训练批调度组件。
...
本方案在确定“独立`K8S`优先，`K8S + Slurm`兜底”路线前，已对主流开源模型开发训练平台和相关基础组件进行调研。调研范围覆盖训练平台、`MLOps`平台、`HPC`门户、开发机、实验管理、镜像仓库、监控日志和训练调度组件，重点关注以下维度：
...
结合当前“独立`K8S`优先 \+ `Volcano`训练执行 \+ 平台控制面 \+ `Slurm`兜底”的业务诉求，继续调研后未发现一个可以完整替代自研控制面的开源训练平台。更可行的方式是按模块选择可复用项目或参考实现。
...
|**模块**|**功能**|**优先级**|**描述**|
|---|---|---|---|
|**训练任务管理**<br>|任务资源规格|P0|优先调研当前算法同学使用资源管理的规格管理方式，并提供兼容性的方式为任务提供指定节点数、单节点`GPU`数、`CPU`、内存、训练镜像和共享存储路径等资源规格。|
||`aitrain CLI`|P0|调研当前算法同学使用CLI进行任务管理的形式，并提供对应的CLI抽象工具。<br>**注意YAML的抽象功能实现。**|
||任务列表<br>|P0|提供训练任务列表页面，支持按用户、状态、时间、任务名称等维度筛选，展示任务状态、资源规格、运行时长、镜像和提交人。<br>需要服务层提供任务管理功能。<br>**本阶段只考虑CLI创建任务，不考虑通过页面创建任务。**|
||任务详情|P0|提供任务详情页，展示任务规格、调度状态、运行实例、节点分布、事件、日志入口、失败原因、输出目录和`SwanLab`实验入口。<br>需要服务层提供任务管理功能。|
|**调度与资源管理**<br>|训练闭环`POC`|P0|在`MVP`正式实施前完成`K8S`训练闭环验证，重点覆盖单机多卡、多机多卡和`RDMA`通信场景。|
||调度后端抽象|P0|平台通过`SchedulerBackend`屏蔽`K8S + Volcano`、`Slurm`等底层差异，上层`Web`、`CLI`、`AI Agent`使用统一任务模型和状态模型。|
||实现`K8S`训练后端|P0|以`K8S + Volcano`作为`MVP`优先训练执行后端，支持将训练任务渲染为`Volcano Job`。|
||任务权限控制|P0|用户在运行生产训练任务时，如果不使用Apptainer，而是Containerd，那么需要控制运行用户权限，避免使用root运行任务|
|**存储基础设施**|存储分层设计与实现|P0|实现多层级的存储管理能力，并推进落地|
||共享存储挂载<br>|P0|开发机和训练任务统一挂载共享存储，保证代码、数据、日志、`checkpoint`和实验记录在开发调试和正式训练中使用一致路径。|
||目录与权限规范|P0|约定`/data/home/{username}`作为用户个人持久化目录，`/data/home/{username}/workspace`作为工作目录，`/data/home/{username}/outputs/{job_id}`作为任务输出目录。<br>**注意用户权限的管理策略，特别是UID和GID如何而来。**|
|**镜像管理**<br>|镜像管理|P0|对接公司既有`Harbor`，支持镜像创建、查询、更新、删除、标签管理、基础环境信息维护和可用性校验。|
||预置镜像|P0|平台提供开发镜像与训练镜像，预置常用框架、`CUDA`、`PyTorch`、`NCCL`、调试工具和`swanlab`包。|
|**实验管理**|`SwanLab`集成<br>|P1|训练框架侧集成`swanlab`包，训练代码生成`SwanLab`实验日志，平台启动任务时注入`SwanLab local`相关环境变量。<br>**给算法同学提供对接SwanLab的功能SDK或者接入文档。**|
||`SwanLab`实验管理|P1|通过`SwanLab`日志查看实验结果；前期通过日志文件规范定义输出路径并在任务详情页展示，后续对接其他存储服务。<br>**需要通过平台页面查看SwanLab实验管理。**|
|**日志管理**<br>|训练日志查看<br>|P0<br>|支持按任务查看训练日志，提供`tail`、关键字检索和基础日志定位能力；`K8S + Volcano`后端优先读取`Pod logs`。<br>先调研当前日志管理能力：是查看进程输出的日志文件，还是终端输出的日志内容？|
|**故障诊断恢复**<br>|故障`GPU`自动检测<br>|P0|**基于指标**：自动采集并识别掉卡、`Xid`、`ECC`、温度、功耗、风扇等异常，形成故障`GPU`和节点健康状态。<br>**基于日志**：基于系统日志、任务日志诊断硬件故障。|
|||||
||故障节点调度剔除|P0|对故障`GPU`卡对应节点、`IB/RDMA`异常节点和存储异常节点进行调度剔除，避免新训练任务继续落到故障资源。|
||存量资源恢复入池|P0|硬件修复后执行健康验收，验收通过后移除隔离标记，将节点重新加入训练调度。|
|**监控告警**<br>|训练资源监控|P0|通过`Prometheus`、`DCGM Exporter`、`kube-state-metrics`等采集节点、`GPU`、训练`Pod`、`Volcano Job`和队列运行情况。|
...
|**平台建设**|多用户管理|P0|对接MAIP现有用户系统，支持登录、注销。|
|**前端开发**|前端开发|P0|平台的前端开发功能。|

### 最低人力预估

预计Q3需要投入的人力，以下为纯研发人力，测试人力估计额外2\-3人，具体里程碑待详细技术方案评审后确定：

投入人力：@郭强@伍佳灵@林海立@王振东@熊云川@刘武@栾瑶

|**主要工作方向**|**人力预估（人）**|**备注**|
|---|---|---|
|**平台建设**<br>**实验管理**|1\.5|用户管理、开发机能力、镜像管理、组件集成、存储集成等基础能力|
||0\.5|SwanLab集成到训练流程、实验日志对接平台展示|
|**资源调度**<br>**训练流程**|0\.5|K8S集群对接、调度后端抽象、故障剔除与恢复|
||1|K8S训练流程、任务管理、训练任务、训练日志对接到平台|
|**故障诊断**|1|故障范围，容错机制，数据发现，采集，分析，处理流程|
|**监控告警**|1|优先考虑硬件故障检测与恢复流程|
...
|**资源治理**|资源治理扩展预留|P2|后续保留项目、队列、资源规格、资源使用记录和审计字段，用于扩展资源配额、资源账本和成本分摊。|
...
## 20260623 交付形式、总体架构、人员分工

1. **交付形式**：MAIP子系统，但保证训练功能的交互在子系统内部能够闭环，避免多系统之间跳转。

2. **总体架构**：采用标准的MAIP架构，训练系统由Web层（Java）\+服务层（Go）构成。

3. **任务排期**：

    1. **0703：主要是调研阶段，输出过程文档，以评估任务进度。**

        - 输出接入层与服务层功能职责边界文档。@伍佳灵

        - 【过程文档】平台功能设计方案，包含各个组件对接流程。@伍佳灵

            - 项目空间

            - 任务管理

            - 镜像管理 @林海立确定镜像维护同学\(是否算法同学提供？\)，什么运行时（containerd/apptainer）

            - CLI相关接口

        - 存储基础设施调研及初步技术方案文档。@郭强

        - 【过程文档】SwanLab实验管理集成及平台的调研文档。@王振东

        - 【过程文档】调度与资源管理：训练闭环POC情况。@林海立

        - 【过程文档】日志管理、故障诊断恢复的调研文档。@熊云川@伍佳灵

        - 【过程文档】监控告警的调研文档。@伍佳灵

    2. **0713：输出技术方案，确定具体里程碑时间。**

        - 平台功能设计技术方案。 @伍佳灵

        - 存储基础设施技术方案。@郭强

        - SwanLab实验管理集成技术方案。@王振东

        - 训练完整流程技术方案。@林海立

        - 日志管理、故障诊断恢复技术方案。@熊云川@伍佳灵
...
|**模块**|**功能**|**优先级**|**描述**|**跟进人**|
|---|---|---|---|---|
|**训练任务管理**|任务资源规格|P0|优先调研当前算法同学使用资源管理的规格管理方式，并提供兼容性的方式为任务提供指定节点数、单节点`GPU`数、`CPU`、内存、训练镜像和共享存储路径等资源规格。|@伍佳灵|
||`aitrain CLI`|P0|调研当前算法同学使用CLI进行任务管理的形式，并提供对应的CLI抽象工具。<br>**注意YAML的抽象功能实现。**||
||任务列表|P0|提供训练任务列表页面，支持按用户、状态、时间、任务名称等维度筛选，展示任务状态、资源规格、运行时长、镜像和提交人。<br>需要服务层提供任务管理功能。<br>**本阶段只考虑CLI创建任务，不考虑通过页面创建任务。**||
||任务详情|P0|提供任务详情页，展示任务规格、调度状态、运行实例、节点分布、事件、日志入口、失败原因、输出目录和`SwanLab`实验入口。<br>需要服务层提供任务管理功能。||
|**调度与资源管理**|训练闭环`POC`|P0|在`MVP`正式实施前完成`K8S`训练闭环验证，重点覆盖单机多卡、多机多卡和`RDMA`通信场景。|@林海立|
||调度后端抽象|P0|平台通过`SchedulerBackend`屏蔽`K8S + Volcano`、`Slurm`等底层差异，上层`Web`、`CLI`、`AI Agent`使用统一任务模型和状态模型。||
||实现`K8S`训练后端|P0|以`K8S + Volcano`作为`MVP`优先训练执行后端，支持将训练任务渲染为`Volcano Job`。||
||任务权限控制|P0|用户在运行生产训练任务时，如果不使用Apptainer，而是Containerd，那么需要控制运行用户权限，避免使用root运行任务||
|**存储基础设施**|存储分层设计与实现|P0|实现多层级的存储管理能力，并推进落地|@郭强|
||共享存储挂载<br>|P0|开发机和训练任务统一挂载共享存储，保证代码、数据、日志、`checkpoint`和实验记录在开发调试和正式训练中使用一致路径。||
||目录与权限规范|P0|约定`/data/home/{username}`作为用户个人持久化目录，`/data/home/{username}/workspace`作为工作目录，`/data/home/{username}/outputs/{job_id}`作为任务输出目录。<br>**注意用户权限的管理策略，特别是UID和GID如何而来。**||
|**镜像管理**|镜像管理|P0|对接公司既有`Harbor`，支持镜像创建、查询、更新、删除、标签管理、基础环境信息维护和可用性校验。|@伍佳灵|
||预置镜像|P0|平台提供开发镜像与训练镜像，预置常用框架、`CUDA`、`PyTorch`、`NCCL`、调试工具和`swanlab`包。||
|**实验管理**|`SwanLab`集成|P1|训练框架侧集成`swanlab`包，训练代码生成`SwanLab`实验日志，平台启动任务时注入`SwanLab local`相关环境变量。<br>**给算法同学提供对接SwanLab的功能SDK或者接入文档。**|@王振东|
||`SwanLab`实验管理|P1|通过`SwanLab`日志查看实验结果；前期通过日志文件规范定义输出路径并在任务详情页展示，后续对接其他存储服务。<br>**需要通过平台页面查看SwanLab实验管理。**||
|**日志管理**|训练日志查看|P0|支持按任务查看训练日志，提供`tail`、关键字检索和基础日志定位能力；`K8S + Volcano`后端优先读取`Pod logs`。<br>先调研当前日志管理能力：是查看进程输出的日志文件，还是终端输出的日志内容？|@熊云川|
|**故障诊断恢复**|故障`GPU`自动检测|P0|**基于指标**：自动采集并识别掉卡、`Xid`、`ECC`、温度、功耗、风扇等异常，形成故障`GPU`和节点健康状态。<br>**基于日志**：基于系统日志、任务日志诊断硬件故障。|@熊云川|
||||||
||故障节点调度剔除|P0|对故障`GPU`卡对应节点、`IB/RDMA`异常节点和存储异常节点进行调度剔除，避免新训练任务继续落到故障资源。||
||存量资源恢复入池|P0|硬件修复后执行健康验收，验收通过后移除隔离标记，将节点重新加入训练调度。||
|**监控告警**|训练资源监控|P0|通过`Prometheus`、`DCGM Exporter`、`kube-state-metrics`等采集节点、`GPU`、训练`Pod`、`Volcano Job`和队列运行情况。|@伍佳灵|
||训练节点磁盘`IO`监控|P0|采集训练节点读写吞吐、`IOPS`、`iowait`、磁盘空间和文件系统异常。||
||`RDMA/IB`网络监控|P0|采集端口状态、链路速率、带宽、错误包、丢包、重传和链路抖动。||
||网络存储监控|P0|采集共享存储挂载状态、容量、吞吐、时延、`IOPS`、错误率和`checkpoint`写入耗时。||
||`GPU`环境指标监控|P0|采集`GPU`温度、功耗、风扇转速、显存、利用率、`ECC`错误和`Xid`错误。||
||异常告警通知|P0|当硬件、网络、存储或训练任务指标异常时，及时通知算法用户和平台运维人员，并在任务详情页展示告警历史。||
|**平台建设**|多用户管理|P0|对接MAIP现有用户系统，支持登录、注销。|@伍佳灵|
|**前端开发**|前端开发|P0|平台的前端开发功能。|@刘武|
