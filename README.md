...
- 训练中心：任务列表 / 新建 / 详情（`Volcano Job`）、我的队列、配置集草稿与版本、实验分析（项目 / `Run` / 机房内`TensorBoard`反代）；任务详情中的日志检索（`ES`）与任务监控（`Grafana`）本迭代为占位

后续变更才会开放：集群概览。本地`kind`通过共享宿主机目录模拟`/data/hpc/home`；在新建任务页使用「填入本地演示训练」提交后，实验分析会展示`Loss`、进度、吞吐，并可打开`TensorBoard`。
