...
本规则约束数据库迁移 SQL、Seed DML、Mock 数据、DAO 生成、数据库幂等执行、自增主键写入策略、索引设计和查询性能基础。

## 数据表设计要求

在设计新的业务模块时，应当和用户确认以下数据表设计点：
...
- 所有交付到`manifest/sql/`、`manifest/sql/mock-data/`的建表、改表、索引变更和数据写入脚本都必须满足可重复执行且结果一致的幂等性要求。
...
- 向自增主键表写入 Seed DML、Mock 数据安装初始化数据时，禁止在`INSERT`列清单中显式指定自增`id`值。
